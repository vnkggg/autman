//[create_at: 2023-11-16 15:39:53]
//[disable:false]
//[author: hdbjlizhe]
// [version: 1.0.5]
// [title: 摸鱼]
//[icon:https://bbs.autman.cn/assets/files/2024-04-11/1712804953-497622-76f4856c-87e7-45fe-8006-cf07248efcff.jpg]
...
// [rule: 摸鱼办]
// [rule: 摸鱼办公室]
//[param:{"required":false,"key":"otto.moyv_push_groups","bool":false,"placeholder":"qqgroup:123,wxgroup:123,tggroup:-100123","name":"推送群","desc":"设置定时推送，格式：qqgroup:123,wxgroup:123,tggroup:-100123"}]

// 是否联网摸鱼，默认离线，联网失败会自动使用离线版本
var isOnline = false
// 黄历群白名单（置空则都展示）
var whiteList = []
// 摸鱼表情包，随机展示
...
function main() {
    ...
    if (whiteList.length == 0 || whiteList.indexOf(GetChatID()) >= 0 || isAdmin()) {
        dateApi();
    }
    if (fishPic.length != 0) {
        msg += image(fishPic[Math.floor(Math.random() * fishPic.length)])
    }

    var imType = ImType();
    if (imType == "fake") {
        // 处理推送群
        var pushGroups = get("moyv_push_groups")
        Debug(pushGroups)
...
function dateApi() {
    ...
    try {
        ...
        msg += "\n————  ┑(￣Д ￣)┍  ————"
        msg += "\n【今日黄历】" + req["yearname"] + "年 " + req["nonglicn"] + " " + req["jieqi"]
        msg += "\n「宜」" + req["suit"].replace(/\./g, "·")
        msg += "\n「忌」" + req["avoid"].replace(/\./g, "·")
    } catch (e) { }
}

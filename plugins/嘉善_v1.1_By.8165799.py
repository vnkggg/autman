# [rule: ^(嘉善)(登录|登陆)$|^登(录|陆)(嘉善)$|^(嘉善)(查询|管理)$|^(查询|管理)(嘉善)$|^嘉善清理$|^嘉善授权$|^嘉善教程$|^嘉善通知 ?(.*)$|^清理嘉善$|^嘉善广播 ?(.*)$]
# [disable:false]
...
# [description: 嘉善代挂提交<br>1. 指令：嘉善登录、嘉善管理、嘉善查询、嘉善授权<br>2. 用户提交 手机号#密码 自动账密登录，未授权账号不写入面板。<br>3. 支持查询最近5天抽奖记录，手机号中间四位脱敏。<br>4. 售后群1003974618。 售后联系：QQ 8165799<br>]
...
try:
    middleware.bucketSet(bucket='jiashan_sender', key=userid, value=str(senderID))
    middleware.bucketSet(bucket='jiashan_imtype', key=userid, value=str(sender.getImtype()))
except:
    pass

# ===================== 插件配置参数 =====================
# [param: {"required":true,"key":"jiashan.panel_type","bool":false,"placeholder":"qinglong/daidai","name":"对接面板类型","desc":"qinglong=青龙面板 daidai=呆呆面板"}]
# [param: {"required":true,"key":"jiashan.jiashan_qlname","bool":false,"placeholder":"Host丨ClientID/AppKey丨Secret","name":"对接系统配置","desc":"青龙:URL丨ID丨Secret 呆呆:URL丨Key丨Secret"}]
# [param: {"required":true,"key":"jiashan.jiashan_osname","bool":false,"placeholder":"默认:JiaShan","name":"系统变量名","desc":"系统容器内变量名(默认为JiaShan)"}]
# [param: {"required":false,"key":"jiashan.epay_alipay","bool":true,"name":"易支付支付宝","desc":"启用易支付支付宝通道收款"}]
# [param: {"required":false,"key":"jiashan.epay_wxpay","bool":true,"name":"易支付微信","desc":"启用易支付微信通道收款"}]
# [param: {"required":false,"key":"jiashan.epay_qqpay","bool":true,"name":"易支付QQ","desc":"启用易支付QQ通道收款"}]
# [param: {"required":false,"key":"jiashan.epay_url","bool":false,"placeholder":"如 http://pay.xxx.com/","name":"易支付网关","desc":"易支付接口网关地址(需带http及结尾/)"}]
# [param: {"required":false,"key":"jiashan.epay_pid","bool":false,"placeholder":"","name":"易支付商户ID","desc":"易支付的PID"}]
# [param: {"required":false,"key":"jiashan.epay_key","bool":false,"placeholder":"","name":"易支付商户密钥","desc":"易支付的KEY密钥"}]
# [param: {"required":false,"key":"jiashan.enable_zsm","bool":true,"name":"个人微信收款","desc":"在支付菜单中增加个人微信收款码方式"}]
# [param: {"required":false,"key":"jiashan.zsm","bool":false,"placeholder":"http://xxxx.co/xxx.jpg","name":"微信收款码链接","desc":"填写个人微信收款码直链，不填则不开启"}]
# [param: {"required":true,"key":"jiashan.zsVipmoney","bool":false,"placeholder":"例:0.88,不填为0元","name":"上车价格","desc":"上车价格(单位:元)/月"}]
# [param: {"required":true,"key":"jiashan.zscoin","bool":false,"placeholder":"不填为关闭积分支付","name":"积分开通","desc":"授权一个月需要多少积分"}]
# [param: {"required":true,"key":"jiashan.enable_proxy","bool":true,"name":"启用代理","desc":"是否启用代理功能"}]
# [param: {"required":false,"key":"jiashan.proxy_pool_url","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
# [param: {"required":true,"key":"jiashan.points_bucket","bool":false,"placeholder":"默认使用dd_sign_points","name":"积分桶名称","desc":"存储用户积分的桶名称"}]
# [param: {"required":true,"key":"jiashan.enable_remark","bool":true,"name":"启用备注功能","desc":"是否启用账号备注功能"}]
# [param: {"required":true,"key":"jiashan.reminder_days","bool":false,"placeholder":"例:2","name":"到期提醒天数","desc":"到期前多少天开始发送提醒通知"}]

def getusercontent():
    """获取插件完整配置"""
    panel_type = middleware.bucketGet('jiashan', 'panel_type') or 'qinglong'
    panel_type = panel_type.lower()
    
    env_qlconfig = middleware.bucketGet('jiashan', 'jiashan_qlname') or ''
    env_name = middleware.bucketGet('jiashan', 'jiashan_osname') or 'JiaShan'
    
    if not env_qlconfig:
        sender.reply("❌ 配置错误：请在插件配置中填写【对接系统配置】(面板信息)。")
        exit(0)
    
    jiashan_managecommand = middleware.bucketGet('jiashan', 'jiashan_managecommand') or '嘉善管理'
    jiashan_querycommand = middleware.bucketGet('jiashan', 'jiashan_querycommand') or '嘉善查询'
    jiashan_signcommand = middleware.bucketGet('jiashan', 'jiashan_signcommand') or '嘉善登录'
    
    enable_proxy = (middleware.bucketGet('jiashan', 'enable_proxy') or 'false').lower() == 'true'
    proxy_pool_url = middleware.bucketGet('jiashan', 'proxy_pool_url') or ''
    points_bucket = middleware.bucketGet('jiashan', 'points_bucket') or 'dd_sign_points'
    enable_remark = (middleware.bucketGet('jiashan', 'enable_remark') or 'false').lower() == 'true'
    
    randommanagecommand = jiashan_managecommand
    randomquerycommand = jiashan_querycommand
    randomsigncommand = jiashan_signcommand
    
    zsVipmoney = Decimal(middleware.bucketGet('jiashan', 'zsVipmoney') or '0')
    zscoin = int(middleware.bucketGet('jiashan', 'zscoin') or '0')
    reminder_days = int(middleware.bucketGet('jiashan', 'reminder_days') or '2')
    
    # 个人微信收款配置
    enable_zsm = (middleware.bucketGet('jiashan', 'enable_zsm') or 'false').lower() == 'true'
    zsm = middleware.bucketGet('jiashan', 'zsm') or ''
    
    # 易支付配置提取
    epay_url = middleware.bucketGet('jiashan', 'epay_url') or ''
    epay_pid = middleware.bucketGet('jiashan', 'epay_pid') or ''
    epay_key = middleware.bucketGet('jiashan', 'epay_key') or ''
    epay_alipay = (middleware.bucketGet('jiashan', 'epay_alipay') or 'true').lower() == 'true'
    epay_wxpay = (middleware.bucketGet('jiashan', 'epay_wxpay') or 'false').lower() == 'true'
    epay_qqpay = (middleware.bucketGet('jiashan', 'epay_qqpay') or 'false').lower() == 'true'
...
def get_owner_user_id(account, fallback_userid=None):
    ...
    try:
        ...
        pass
    try:
        for owner in middleware.bucketAllKeys(bucket='jiashan_user'):
            try:
...
def send_daily_admin_report(report_data, force_send=False, notify_status=False):
    ...
    if not force_send and middleware.bucketGet('jiashan_runtime', report_key):
        if notify_status:
    ...
    if framework_sent:
        try:
            middleware.bucketSet('jiashan_runtime', report_key, "framework")
        except Exception:
            pass
        if notify_status:
...
def batch_verify_account_ck(tasks, max_workers=MAINTENANCE_CK_MAX_WORKERS):
    # 嘉善为账密登录，接口偶发波动时容易误判。为避免误推用户，默认不做失效推送。
    return {}

def send_user_notice(user_id, msg, title="嘉善通知"):
    try:
        imtype = middleware.bucketGet(bucket='jiashan_imtype', key=str(user_id)) or sender.getImtype()
    except:
    ...
    targets = []
    try:
        saved_sender = middleware.bucketGet(bucket='jiashan_sender', key=str(user_id))
        if saved_sender:
...
def parse_panel_jiashan_remark(remarks):
    try:
    ...
    try:
        ...
        for env in sys_api.get_all_envs():
            if env.get('name') != config['env_name']:
                continue
            stats['scanned'] += 1
            info = parse_panel_jiashan_remark(env.get('remarks') or env.get('remark') or '')
            if not info:
                continue
            AccountManager.add_account(info['user'], info['account'])
            middleware.bucketSet(bucket='jiashan_auth', key=info['account'], value=info['auth_date'])
        ...
        logger.warning(f"嘉善面板授权回写本地失败: {e}")
    return stats
...
class RemarkManager:
    ...
    def get_account_remark(user_id, account_id):
        try:
            remark_data = middleware.bucketGet(bucket='jiashan_remarks', key=f'{user_id}_{account_id}')
    ...
    def set_account_remark(user_id, account_id, remark):
        try:
            ...
            if remark_clean:
                middleware.bucketSet(bucket='jiashan_remarks', key=f'{user_id}_{account_id}', value=remark_clean)
                return remark_clean
            return ""
        except: return ""
    ...
    def delete_account_remark(user_id, account_id):
        try:
            middleware.bucketDel(bucket='jiashan_remarks', key=f'{user_id}_{account_id}')
            return True
        except: return False
...
class AccountManager:
    ...
    def get_accounts(user_id):
        try:
            value = middleware.bucketGet(bucket='jiashan_user', key=str(user_id))
            if not value: return []
    ...
    def add_account(user_id, account):
        try:
            ...
            if account not in accounts:
                ...
                middleware.bucketSet(bucket='jiashan_user', key=str(user_id), value=str(accounts))
                return True
            return False
        except: return False
        ...
        try:
            ...
            if account in accounts:
                ...
                if accounts:
                    ...
                    middleware.bucketDel(bucket='jiashan_user', key=str(user_id))
                return True
            return False
        except: return False
    ...
    def update_account_token(account, token):
        try:
            ...
            middleware.bucketSet(bucket='jiashan_token', key=str(account), value=encrypted_token)
            return True
        except: return False
    ...
    def update_account_mobile(account, mobile):
        try:
            mobile = str(mobile or "").strip()
            if mobile:
                middleware.bucketSet(bucket='jiashan_mobile', key=str(account), value=mobile)
                return True
            return False
        except: return False
    ...
    def get_account_mobile(account):
        try:
            return middleware.bucketGet(bucket='jiashan_mobile', key=str(account)) or ""
        except: return ""
    ...
    def update_account_web_token(account, web_token):
        try:
            ...
            if web_token:
                middleware.bucketSet(bucket='jiashan_web_token', key=str(account), value=encrypt_token(web_token))
                return True
            return False
        except: return False
    ...
    def get_account_web_token(account):
        try:
            enc = middleware.bucketGet(bucket='jiashan_web_token', key=str(account))
    ...
    def get_token(account):
        try:
            enc = middleware.bucketGet(bucket='jiashan_token', key=str(account))
    ...
    def get_all_users():
        try:
            users = middleware.bucketAllKeys(bucket='jiashan_user')
            user_list = []
            for user in users:
    ...
    def migrate_account(user_id, old_account, new_account, new_token, remark=""):
        try:
            ...
            old_vip = middleware.bucketGet(bucket='jiashan_auth', key=old_account)
            new_vip = middleware.bucketGet(bucket='jiashan_auth', key=new_account)
            if old_vip and (not new_vip or str(old_vip) > str(new_vip)):
                middleware.bucketSet(bucket='jiashan_auth', key=new_account, value=old_vip)

            old_bind_date = middleware.bucketGet(bucket='jiashan_bind_date', key=old_account)
            if old_bind_date and not middleware.bucketGet(bucket='jiashan_bind_date', key=new_account):
                middleware.bucketSet(bucket='jiashan_bind_date', key=new_account, value=old_bind_date)
            ...
            middleware.bucketSet(bucket='jiashan_user', key=str(user_id), value=str(new_accounts))

            AccountManager.update_account_token(new_account, new_token)
            try: middleware.bucketDel(bucket='jiashan_token', key=old_account)
            except: pass
            try: middleware.bucketDel(bucket='jiashan_mobile', key=old_account)
            except: pass
            try: middleware.bucketDel(bucket='jiashan_web_token', key=old_account)
            except: pass
            try: middleware.bucketDel(bucket='jiashan_auth', key=old_account)
            except: pass
            return True
        except Exception as e:
...
def process_single_account_query(account, index, total_count, account_remarks):
    try:
        ...
        accountVip = middleware.bucketGet(bucket='jiashan_auth', key=account)
...
==================""")
        ...
        for i, acc in enumerate(accounts, 1):
            ...
            vip = middleware.bucketGet(bucket='jiashan_auth', key=acc)
            if not vip:
                vip_tag = '⚠️未授权'
            elif vip < today_time:
                vip_tag = '❌已过期'
        ...
        # 嘉善服务器偶发掐断连接，控制并发数避免瞬时连接过多触发服务端拒绝
...
def notify_authorized_users():
    ...
    content = ""
    match = re.search(r'^\s*(嘉善广播|嘉善通知)\s*(.*)$', usermessage, re.S)
    if match:
        content = match.group(2).strip()
    
    if not content:
        sender.reply("❌ 请输入通知内容，例如：嘉善通知 系统维护中")
        return
    ...
    try:
        ...
        for uid in all_users:
            ...
            for acc in user_accounts:
                vip_date = middleware.bucketGet(bucket='jiashan_auth', key=str(acc))
                if vip_date and vip_date >= today:
                    has_auth = True
                    break
            
            if has_auth:
                ok, err = send_user_notice(uid, f"📢 【嘉善管理员通知】\n\n{content}", "嘉善管理员通知")
                if ok:
                    success_count += 1
                    time.sleep(0.3)
                else:
                    fail_count += 1
                    logger.warning(f"嘉善管理员通知发送失败 {uid}: {err}")
...
def process_account_binding(full_token, unique_id, nickname, remark="", aliases=None, acc_type="", legacy_key="", mobile="", web_token="", silent=False):
    try:
        ...
        if is_new:
            try: middleware.bucketSet(bucket='jiashan_bind_date', key=account, value=str(datetime.now().date()))
            except: pass
        ...
        if choice == '1':
            ...
            if process_payment(months, accountVip, token, account, remark):
                try:
                    ...
                    try: middleware.bucketSet(bucket='jiashan_auth', key=account, value=new_auth_time)
                    except: pass

                    today_date = datetime.now().date()
                    for d in range(config['reminder_days'] + 1):
                        remind_key = f"{userid}_{account}_{today_date - timedelta(days=d)}"
                        try: middleware.bucketDel('jiashan_remind_log', remind_key)
                        except: pass
            ...
            if get_user_input() == 'y':
                try:
                    ...
                    try: middleware.bucketDel(bucket='jiashan_token', key=account)
                    except: pass
                    try: middleware.bucketDel(bucket='jiashan_mobile', key=account)
                    except: pass
                    try: middleware.bucketDel(bucket='jiashan_web_token', key=account)
                    except: pass
                    try: middleware.bucketDel(bucket='jiashan_auth', key=account)
                    except: pass
...
def batch_auth_selected(accounts, account_remarks):
    ...
    for account in accounts:
        try:
            ...
            try: middleware.bucketSet('jiashan_auth', account, new_date)
            except: pass
        ...
        try:
            ...
            for account_item in context["accounts"]:
                ...
                if days_diff >= 0:
                    ...
                    # 嘉善不做 CK/配置失效提醒，避免接口不稳定导致误推。
                    is_ck_valid = True
                    
                    if is_ck_valid and 0 <= days_diff <= reminder_days_cfg:
                        remind_key = f"{user}_{account}_{today_date}"
                        has_reminded = middleware.bucketGet('jiashan_remind_log', remind_key)
...
=================="""
                            if safe_send_message(user, msg, f"到期提醒 {user}-{account}"):
                                try: middleware.bucketSet('jiashan_remind_log', remind_key, "1")
                                except: pass
                                reminded_count += 1
                    continue
...
=================="""
            ...
            if user_has_change:
                if valid_accounts:
                    try: middleware.bucketSet(bucket='jiashan_user', key=str(user), value=str(valid_accounts))
                    except: pass
                else:
                    try: middleware.bucketDel(bucket='jiashan_user', key=str(user))
                    except: pass
    ...
    for user in users:
        for account in AccountManager.get_accounts(user):
            try:
                ...
                if not token:
                    stats["no_token"] += 1
                vip = middleware.bucketGet(bucket='jiashan_auth', key=account)
                if not vip:
...
def admin_user_ck_preview():
    ...
    for i, row in enumerate(rows, 1):
        ...
        if row["no_token"]:
            extra.append(f"缺账密{row['no_token']}")
    ...
    if choice == '1':
        ...
        if len(users) <= 1 and total_accounts > 5:
            sender.reply(
                ...
                "请先恢复 jiashan_user 归属表，或使用 [2] 同步指定用户账号。"
            )
            return
...
def admin_auth_all_users():
    ...
    for user in all_users:
        ...
        for account in accounts:
            try:
                account = str(account)
                accVip = middleware.bucketGet(bucket='jiashan_auth', key=account)
                new_vip = empower(accVip, days)
                try: middleware.bucketSet(bucket='jiashan_auth', key=account, value=new_vip)
                except: pass
...
def admin_auth_specific_user():
    ...
    for i, acc in enumerate(target_accounts, 1):
        acc = str(acc)
        accVip = middleware.bucketGet(bucket='jiashan_auth', key=acc)
    ...
    for acc in selected_accs:
        ...
        accVip = middleware.bucketGet(bucket='jiashan_auth', key=acc)
        new_vip = empower(accVip, days)
        try: middleware.bucketSet(bucket='jiashan_auth', key=acc, value=new_vip)
        except: pass

...
# [rule: ^今视频(.*)|(.*)今视频$] 匹配规则，多个规则时向下依次写多个
# [cron: 46 8,18 * * *] cron定时，支持5位域和6位域
# [priority: 99999999] 优先级，数字越大表示优先级越高
# [platform: all] 适用的平台
# [open_source: false]是否开源
# [icon: http://img.jxdown.com/upload/2026-4/2026429923454752.jpg]图标链接地址，请使用48像素的正方形图标，支持http和https
# [version: 1.0.0]版本号
# [public: true] 是否发布？值为true或false，不设置则上传aut云时会自动设置为true，false时上传后不显示在市场中，但是搜索能搜索到，方便开发者测试
# [price: 18.88] 上架价格
# [description: 无脚本提供；支持扫码登录和抓包批量登录，ck提交青龙，格式：token<br>指令：今视频(登录|登陆|上车|查询|管理|授权|清理)<br>支持积分支付授权,可自定义积分数据桶。<br>1.0.0初版：支持授权、批量登录、代理、积分开通等功能] 使用方法尽量写具体



# [param: {"required":true,"key":"mrconli.jinshipin.bind","bool":false,"placeholder":"0","name":"登录方式","desc":"0：所有方式，1：仅扫码登录，2：仅CK登录"}]
...
# [param: {"required":true,"key":"mrconli.jinshipin.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"|"}]
# [param: {"required":false,"key":"mrconli.jinshipin.var_name","bool":false,"placeholder":"m_jinshipin","name":"环境变量名","desc":"青龙容器内的变量名，默认为：m_jinshipin"}]
# [param: {"required":false,"key":"mrconli.jinshipin.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.jinshipin.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.jinshipin.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]
# [param: {"required":false,"key":"mrconli.jinshipin.is_proxy","bool":true,"placeholder":"","name":"是否启用代理","desc":"开启代理就勾选，其实不需要代理"}]
# [param: {"required":false,"key":"mrconli.jinshipin.proxy_pool","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
...
def get_token_by_code(code):
    ...
    #    'Cookie': 'zycna=ys+zineugMMBARsdtJTF8CBE; acw_tc=2f6b79ca17772241012028568ee54122b3e481a813dfeb43dff066a6bac19e',
        'Host': 'app.jxgdw.com',
        'appVersion': '6.1.2',
...
def get_config():
    """获取插件配置"""
    try:
        var_name = middleware.bucketGet(bucket_prefix, 'var_name') or "m_jinshipin"
        if not var_name:
    ...
    try:
        if choice == '01':
            ...
            try:
                ...
                sender.reply(f"正在为账号{start_num}-{end_num}进行批量授权...")
                perform_batch_auth(selected_accounts, days=None)
            except ValueError:
            ...
            # 多选账号授权逻辑
            try:
                selected_indices = [int(idx.strip()) - 1 for idx in choice.replace('，', ',').split(',')]
                selected_indices = [idx for idx in selected_indices if 0 <= idx < len(accounts)]
...
def tutorial():
    ...
    tutorial_text = (
        ...
        "📝 入口:\n"
        f"    应用商店下载“{full_scripts_name}”app\n"
        "🌟 基础指令:\n"

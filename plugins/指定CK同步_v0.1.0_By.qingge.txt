...
//[priority: 9999999999]
//[service: 97393412]
//[description: 说明:指定PIN。。非指定的不会同步,增加指定模式开关,支持使用指定CK数量以及上传有效CK和从N个CK开始上传,增加多容器同步,同步CK数量一致]
// [param: {"required":true,"key":"who_tong.zdtbmc","placeholder":"容器名称","name":"源青龙名称","desc":"获取名方法:奥特曼后台-容器管理--对接容器--名称"}]
// [param: {"required":true,"key":"who_tong.zdtbql","placeholder":"同步到容器","name":"同步到容器","desc":"填写你需要同步CK到XX的名称,以小写,号分割"}]
// [param: {"required":true,"key":"who_tong.zdbzsm","placeholder":"","name":"备注","desc":"CK上传备注"}]
// [param: {"spliter":true}]
// [param: {"required":false,"key":"who_tong.zdck_kg","bool":true,"placeholder":"false","name":"模式开关","desc":"默认指定PIN同步，打开则只同步指定数量CK"}]
// [param: {"required":true,"key":"who_tong.zdcktb","placeholder":"","name":"指定PIN","desc":"填你需要指定同步的pin,用,逗号分割"}]
// [param: {"required":true,"key":"who_tong.zdck_cong","placeholder":"","name":"从N个CK开始","desc":"从第?个CK开始，该功能需要开启模式开关"}]
// [param: {"required":true,"key":"who_tong.zdck_len","placeholder":"","name":"同步CK数量","desc":"该功能需要开启模式开关"}]
// [param: {"required":false,"key":"who_tong.zdck_yx","bool":true,"placeholder":"false","name":"有效CK","desc":"默认不检测CK状态,打开则只上传有效CK"}]
//[cron: 5 1,11 * * *]
var GetContent = GetContent()
var imType = ImType()
var checkJS = false
var jsonObject = []//加入数据
var zdbzsm = bucketGet("who_tong", "zdbzsm")
var jd_guolv = bucketGet("who_tong", "zdcktb")//过滤
var zdck_kg = bucketGet("who_tong", "zdck_kg")//模式
var zdck_len = parseInt(bucketGet("who_tong", "zdck_len"))//同步数量
var zdck_cong = parseInt(bucketGet("who_tong", "zdck_cong"))//从N个CK开始
var zdck_yx = bucketGet("who_tong", "zdck_yx")//检测状态
try {
...
function mian() {
    ...
    var ql_name = bucketGet("who_tong", "zdtbmc")
    ...
    let containeta = qls(ql_name)
    let container = ""
    let containerEnv = "", cookieObj = ""
    var QW = 0
    var ck_len = 0
    for (let T = 0; T < 10; T++) {
        try {
            ...
            cookieObj = containerEnv.filter(function (_data) {
                return _data.name == "JD_COOKIE" && _data.status == 0
            });

            T = 15
        } catch (e) {
            notifyMasters("【指定CK同步1】链接容器【" + ql_name + "】失败，请检查配置是否正确或网络是否正常")
            //return
        }
    ...
    Debug(`总CK数量：${cookieObj.length},从：${QW}开始${zdck_cong},同步数:${zdck_len},${zdck_kg},${zdck_yx}`)
    for (QW = QW; QW < cookieObj.length; QW++) {//ckx.length

        var pina = cookieObj[QW].value.match(/(?<=pt_pin=)[^;]+/g)
        if (zdck_kg == "true") {
            //Debug(ck_len)
            if (zdck_yx == "true") {
                //Debug(`当前数量是${ck_len},设定${zdck_len}...`)
                if (jdd_ck(cookieObj[QW].value)) {
                    // Debug(`${ck_len},${cookieObj[QW].value}`)
                    ck_len = ck_len + 1
                    jsonObject.push(cookieObj[QW].value)
                    if (ck_len == zdck_len) {
...
function subCKToXZ() {
    // try {
    let ck = mian()
    Debug(`获取到CK数量：${ck.length}`)
    //let name =xianbao
    //获取青龙容器名
    var zdtbql = bucketGet("who_tong", "zdtbql")
    let tz_zd = zdtbql.split(",")
    for (let g = 0; g < tz_zd.length; g++) {
        let containeta = qls(tz_zd[g])
        let container = ""
        let containerEnv = ""
        for (let T = 0; T < 5; T++) {
            try {
                container = Qinglong(containeta.host, containeta.client_id, containeta.client_secret)
                //sendText(JSON.stringify(container) + "---1")
                containerEnv = container.ApiQL("envs", "", "get", "").data
                //Debug(JSON.stringify(containerEnv))
                ...
                for (let j = 0; j < ck.length; j++) {
                    ...
                    var userId = ck[j].match(/(?<=pt_pin=)[^;]+/g)
                    //Debug(`${userId}---${i}`)
                    let cookieObj = containerEnv.filter(function (_data) {
                        return _data.name == "JD_COOKIE" && _data.value.indexOf(userId) != -1
                    });
                    // 判断有没有找到相应ck对象
                    if (cookieObj.length == 0) {
                        ...
                        msg = container.ApiQL("envs", "", "post", JSON.stringify(data))
                        if (msg.code == 200) {
                            Debug(userId + "--添加成功")
                            xztb = xztb + 1
                        }
                        ...
                        msg = container.ApiQL("envs", "", "put", JSON.stringify(data))

                        if (msg.code == 200) {
                            Debug(userId + "--更新成功")
                            container.ApiQL("envs/enable", "", "put", `[${cookieObj[0].id}]`)//失效envs/disable
                            gxtb = gxtb + 1
                        }
                    }
                }
                sendText(`容器：${tz_zd[g]}\n同步CK,本轮同步CK:${gxtb + xztb}\n新增CK数:${xztb}\n更新CK数:${gxtb}\n设定CK数:${zdck_len}`)
                notifyMasters(`容器：${tz_zd[g]}\n同步CK,本轮同步CK:${gxtb + xztb}\n新增CK数:${xztb}\n更新CK数:${gxtb}\n设定CK数:${zdck_len}`)
                T=9
            } catch (e) {
                notifyMasters(`【指定CK同步】链接容器失败，请检查配置是否正确或网络是否正常,正在尝试第${T}`)
                return
            }

...
# [description: 插件自带任务! 使用前请先配参。用户指令: 粉象上车,粉象管理,粉象查询,粉象教程；管理员指令: 粉象版本,粉象运行,粉象配置,粉象授权。运行在定时推送里面设置自动运行时间。建议每天21点前运行一次，22点左右运行一次!相关配置桶: bd_fxconfig 用户数据桶: bd_fxcks]
...
if __name__ == '__main__':
    ...
    cjname = '彼得_粉象生活'
    version = 'V6.25'
    middleware.bucketSet('bd_cjbb', f'{cjname}', f'{version}')
    if message == '粉象上车':
        atm_fx.fxsc()
    elif message == '粉象管理':
        atm_fx.fxgl()

...
//[priority: 9999999999]
//[service: 97393412]
//[description: 数据适配作者[specter]的数据,登陆成功后CK交给奥特曼内部同步到容器,需要(qls))权限<br/>增加群组使用开关<br/> 12.5 增加批量导入指令：socks导入<br/> <br/>3.5 修复短信登录问题]
...
// [param: {"spliter":true}]
// [param: {"required":false,"key":"who_tong.bbk_socks","bool":true,"placeholder":"false","name":"socks5模式开关","desc":"打开则开启单用户使用,默认不使用"}]
...
// [param: {"required":true,"key":"who_tong.bbk_base64","bool":true,"placeholder":"","name":"二维码开关","desc":"默认使用二维码的方式发送,打开则使用链接+文案分开发送,如果二维码发送不出来的可以使用这个看看"}]
...
var bbk_base64 = bucketGet("who_tong", "bbk_base64")//青龙
...
var bbk_socks = bucketGet("who_tong", "bbk_socks")//限制同时登陆
...
var cloud_username = bucketGet("cloud", "username")//开启过滤黑名单
var bbk_jk = "http://106.52.219.147:8080"//BBK内置接口
//------------------------------
...
var PIN_HB = []//合并过期PIN
var loginA = {}
var encryptedPwd = "", bbk_add_yz = false, qt_name = ""
var cuowu = false
//----------------------------
var cg = 0//登录成功
var dx = 0//出现短信
        ...
        if (cookieObj.length == 0) {
            ...
            if (pin_ck.indexOf("account") != -1) {
                ...
                if (login) {
                    ...
                    handleLoginSuccess(elmckUserIdArr[k])
                    login = false
                }
            } else {
                qtbl = qtbl + 1
    ...
    for (let k = 0; k < elmckUserIdArr.length; k++) {
        ...
        if (cookieObj.length == 0) {
            ...
            if (pin_ck.indexOf("account") != -1) {
                ...
                pwdLoginApi(userId, pin_ck.account, pin_ck.password)
                //调用青龙
                if (login) {
...
function pwdLoginApi(PIN, username, password) {
    let socks_ipxx = bucketGet("Auto_IP", username)
    if (bbk_socks == "true") {
        if (bbk_socks_ms == "true") {
            if (socks_ipxx == "") {
                socks5proxy = socks_mian("socks5-get", username)
            } else {
                console.warn(`用户:${username},已经分配过:${socks_ipxx}`)
                socks5proxy = socks_ipxx
            }
        } else {
            if (socks_ipxx == "") {
                //发现该手机号并未分配到IP,现在开始分配IP

                socks5proxy = socks_hq(username)
                bucketSet("Auto_IP", username, socks5proxy)
            } else {
                console.warn(`用户:${username},已经分配过:${socks_ipxx}`)
                socks5proxy = socks_ipxx
            }
    ...
    for (let i = 0; i < 3; i++) {
        ...
        if (body) {
            ...
            if (body.msg == "登录失败:账号或密码不正确" || body.msg == "账号或密码不正确") {
                ...
                mmtj = mmtj + 1
                if (GetContent == "账密登录" || GetContent == "账密登陆" || GetContent == "帐密登陆" || GetContent == "帐密登陆" || GetContent == "登陆" || GetContent == "登录") {
                } else {
...
登录失败->${body.msg}`)
            ...
            if (body.msg == "访问服务器出错" || body.msg == "请求超时" || body.msg == "check1请求失败,请重新触发登录验证" || body.msg == "页面已超时，请返回重试" || body.msg == "sid请求失败,请重新触发登录验证" || body.msg == "API请求Timeout失败，请检查网路重试" || body.msg == "refresh验证失败,请重新触发登录验证" || body.msg == "sid请求失败,登录错误,请重试" || body.msg == "执行错误了哦" || body.msg == "请求没有返回数据" || body.msg == "服务器链接失败了哦" || body.msg == "请重新触发登录验证" || body.msg == "接收内容为空") {
...
请您重新发送:账密登录`)
            ...
            if (body.msg == "操作过于频繁，请24小时后再试，或先使用其他方式登录" || body.code == 429) {
                i = 5
                get_tx(`账号->${decodeURIComponent(PIN)}\n-->登录失败,请您先使用短信验证码登录`)
                quchong(decodeURIComponent(PIN) + ":登录失败,原因:" + body.msg, decodeURIComponent(PIN))
                sb = sb + 1
            ...
            if (body.code == 200) {
                ...
                if (msg.indexOf("非白名单") != -1) {
                    ...
                    if (msg.indexOf("为了您的账号安全请到京东商城") != -1 || msg.indexOf("打开京东商城APP重新登录") != -1) {
                        ...
                        get_tx(`账号->${decodeURIComponent(PIN)}
失败原因->需要登录APP验证，请去京东app登录过验证以后
APP登录一次过下人脸，再来登录！后期就稳了！
再发送:账密登录`)
...
function spwdLoginApi(PIN, username, password, kgkz) {
    ...
    let shuaxin = true
    let cloud = bucketGet("cloud", "username")//开启过滤黑名单
    let bbk_cs = 0
    for (let i = 0; i < 3; i++) {

        if (bbk_socks == "true") {

            var obj = { username, password, socks5proxy, shuaxin, cloud };
        } else {
            var obj = { username, password, shuaxin, cloud };
        }
        let qd = stxs.split(",");
        let body = request({
            method: "post",
        ...
        if (msg == "登录失败:账号或密码不正确" || msg == "账号或密码不正确") {
            ...
            mmtj = mmtj + 1
            if (!(GetContent == "账密登录" || GetContent == "账密登陆" || GetContent == "帐密登陆" || GetContent == "登陆" || GetContent == "登录")) {
                for (let k = 0; k < qd.length; k++) {
        ...
        if (msg == "强风控账号,登陆失败" || msg == "您的账号在当前应用已注销，无法继续使用，如需使用请重新注册新账号") {
            i = 5
            get_tx(`账号->${decodeURIComponent(PIN)}\n-->${msg}`)
            quchong(decodeURIComponent(PIN) + ":登录失败,原因:" + msg, decodeURIComponent(PIN))
            sb = sb + 1
        ...
        if (body.code == 200) {
            ...
            bucketSet("pin" + imType.toUpperCase(), ck_pin, GetUserID());
            bucketSet("who_pro_hmd", ck_pin);
            if (bbk_usertz != "true" && (GetContent == "账密刷新" || GetContent == "B账密刷新" || GetContent == "登陆" || GetContent == "登录")) {
                for (let k = 0; k < qd.length; k++) {
        ...
        if (msg.indexOf("非白名单") != -1) {
            ...
            if (bbk_base64 == "" || bbk_base64 == "false") {
            ...
            if (kgkz == false) {
                ...
                var w = asse(`账号->${decodeURIComponent(PIN)}\n-->${fsyzfs},然后使用京东APP二维码识别，并完成验证，验证码/人脸识别验证\n-->验证后在回复：1`);
            } else {
                var w = asse(`账号->${decodeURIComponent(PIN)}\n-->${fsyzfs},然后使用京东APP二维码识别，并完成验证，验证码/人脸识别验证\n-->过完验证后，重新发送:账密登录`);
            }
            ...
            return;
        } else if (msg.indexOf("操作过于频繁，请24小时后再试") != -1 || msg.indexOf("操作过于频繁证") != -1 || body.code == 429) {
            ...
            cuowu = true;
            return;
        } else if (msg.indexOf("check1请求失败,请重新触发登录验证") != -1 || msg.indexOf("请求超时") != -1 || msg.indexOf("页面已超时，请返回重试") != -1 || msg.indexOf("refresh验证失败,请重新触发登录验证") != -1 || msg.indexOf("访问服务器出错") != -1 || msg.indexOf("接收内容为空") != -1 || msg.indexOf("登录错误") != -1 || msg.indexOf("sid请求失败,请重新触发登录验证") != -1 || msg.indexOf("执行错误了哦") != -1 || msg.indexOf("请求没有返回数据") != -1 || msg.indexOf("服务器链接失败了哦") != -1) {
            if (bbk_socks_ms == "true") {
                socks5proxy = socks_mian("socks5-get", username)
            }
            bbk_cs++
            ...
            if (GetContent == "账密登录" || GetContent == "账密登陆" || GetContent == "帐密登陆" || GetContent == "帐密登录" || GetContent == "登陆" || GetContent == "登录") {
                get_tx(`账号->${decodeURIComponent(PIN)}\n失败原因->需要登录APP验证，请去京东app登录过验证以后\nAPP登录一次过下人脸，再来登录！后期就稳了！\n再发送:账密登录`);
            }
            quchong(`${decodeURIComponent(PIN)},原因:人脸`, decodeURIComponent(PIN));
            for (let k = 0; k < qd.length; k++) {
...
-->绑定客户：${userId}(${imType})`)
                ...
                handleLoginSuccess(bind[choice - 1])
                login = false
            }
...
function bbk() {
    if (cloud_username == "") {
        notifyMasters("权限不足,请先给【cloud数据】权限")
        return
    }
    if (GetContent == "短信登录" || GetContent == "短信登陆") {
        zack1_dx()
    }
    if (GetContent == "登陆" || GetContent == "登录" || GetContent == "更新账号") {
        let p = asse(`请选择登录方式:
回复()内的数字编号即可
------------  
...
（输入“q”即可退出会话）`)
    ...
    for (let k = 0; k < 1; k++) {
        try {
            ...
            var login_ds = zack1_sms(p)
            if (login_ds.status == "fail") {
                get_tx("代理失效,请您重新发送：登录")
                // proxyIp = xk()
                k = 0
            } else if (login_ds.err_code == 200 || login_ds.err_code == 0) {
                k = 5
                // get_tx(login_ds.msg)
                ...
                if (w == false||w=="q"||w=="Q") {
                    ...
                    //get_tx(`${p}-${w}`)
                    let sms_login = zack1_smsVerify(p, w, login_ds.guid, login_ds.token, login_ds.appid)
                    //console.log(JSON.stringify(sms_login))
                    if (sms_login.err_code == 200 || sms_login.err_code == 0) {
                        let str = sms_login.cookie

                        ck_pin = str.match(/(?<=pt_pin=)[^;]+/g)
                        let bbk_dxzdynr = bucketGet("who_tong", "bbk_dxzdynr");
                        get_tx(`报告老板，[ ${decodeURIComponent(ck_pin)} ],登录成功\n${bbk_dxzdynr}`)

                        // 截取方括号之间的内容
                        notifyMasters(`-->报告老板，[ ${ck_pin} ]短信更新账号！\n-->绑定客户：${userId}(${imType})`)
                        ...
                        if (sms_login.msg || sms_login.err_msg || "请求结果未知" == "验证码输入错误") {
                            ...
                            get_tx(`${sms_login.msg || sms_login.err_msg || "请求结果未知"},请您重新发送：登录`)


                        } else if (sms_login.err_code == 118) {
                            get_tx(`请您先去京东APP上登录,如果登录还是出现这个，请您使用验证码登录,使用3的方式登录`)
                        } else if (sms_login.err_code == 203) {
                            get_tx(sms_login.msg || sms_login.err_msg || "请求结果未知")
...
function bbk_dx() {
    ...
    if (p == false) {
        ...
        let login_ds = bbk_sms(p)
        if (login_ds.code == 200) {
            // get_tx(login_ds.msg)
            let w = asse(login_ds.msg)
            if (w == false) {
...
function zack1_smsVerify(mobile, smsCode, guid, token, appid) {
    //mobile, smsCode, guid, token, appid
    ...
    for (let i = 0; i < 3; i++) {
        ...
        let shuaxin = true
        let sms = true
        let cloud = cloud_username
        var obj = { mobile, smsCode, shuaxin, sms, cloud, guid, token, appid };
        let body = request({
            method: "post",
            url: `${bbk_jk}/xcx/pwdLoginApi`,
            body: JSON.stringify(obj),
...
function xk() {
    let body = request({
        url: "https://service.ipzan.com/core-extract?num=1&no=20231213653545355112&minute=3&repeat=1&pool=ordinary&mode=whitelist&secret=lh089aonhuod9jo",
        method: "get",
        // body: water,
        headers: {

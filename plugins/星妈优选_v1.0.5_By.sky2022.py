...
# [price: 12.88] 上架价格
# [description: 出自徒弟：GFAN<br><br>📱 <b>功能特色：</b><br>• 多账号批量管理，支持无限绑定<br>• 自动签到 + 自动完成每日任务<br>• 智能token刷新，无需手动维护<br>• 双重支付方式：积分支付 + 微信支付<br>• 实时积分查询，账号状态一目了然<br><br>💡 <b>核心指令：</b><br>🔐 星妈登录 - 快速绑定账号<br>📊 星妈查询 - 查看积分余额<br>⚙️ 星妈管理 - 账号授权管理<br>🚀 星妈一键运行 - 批量执行任务<br>👑 星妈授权 - 管理员批量授权<br><br>✨ 适配呆呆积分系统，支持积分自动扣费<br>🔄 版本1.0.0 稳定版，持续更新优化中]

# 插件参数配置
# [param: {"required":false,"key":"G_xmyx_config.zsm","bool":false,"name":"收款方式","placeholder":"http://example.com/pay.jpg","desc":"微信赞赏码/收款码链接。未填写时会回退读取 G_SKM.zsm 或 dd_sign_config.zsm"}]
# [param: {"required":false,"key":"G_xmyx_config.price","bool":false,"name":"上车价格","placeholder":"例:0.88,填0为免费","value":"0.88","desc":"单个账号每月授权价格，按月数和账号数量累计"}]
# [param: {"required":false,"key":"G_xmyx_config.points_per_month","bool":false,"name":"积分开通","placeholder":"例:100,填0为关闭积分支付","value":"100","desc":"单个账号每月消耗积分，按月数和账号数量累计"}]
# [param: {"required":false,"key":"G_xmyx_config.use_ma_pay","bool":true,"name":"使用码支付","desc":"是否使用码支付系统，开启后将使用卡密系统中的全局码支付配置"}]
...
appKey = (
    middleware.bucketGet(bucket=BUCKET_CONFIG, key="appKey")
...
def get_bucket_text(bucket, key, default=""):
    """读取字符串配置并清理空白"""
    return str(middleware.bucketGet(bucket=bucket, key=key) or default).strip()
...
def get_pay_config():
    """读取全局码支付配置"""
    pay_types_raw = (
        get_bucket_text("dd_sign_config", "pay_types")
        or get_bucket_text("dd_sign_config", "ma_pay_type")
    )
    pay_types = parse_pay_types(pay_types_raw)
    config = {
        "zsm": get_bucket_text("G_SKM", "zsm") or get_bucket_text("dd_sign_config", "zsm"),
        "ma_pay_switch": parse_bool(get_bucket_text("dd_sign_config", "ma_pay_switch")),
        "gateway": get_bucket_text("dd_sign_config", "ma_pay_gateway"),
        "pid": get_bucket_text("dd_sign_config", "ma_pay_pid"),
        "key": get_bucket_text("dd_sign_config", "ma_pay_key"),
        "type": get_bucket_text("dd_sign_config", "ma_pay_type"),
        "notify_url": get_bucket_text("dd_sign_config", "ma_pay_notify_url"),
        "return_url": get_bucket_text("dd_sign_config", "ma_pay_return_url"),
        "pay_types": pay_types,
    }
...
class MaPayClient:
    ...
    def create_order(self, amount, pay_type_key, out_trade_no, subject, attach_value=""):
        ...
        try:
            ...
            self.last_error = f"创建支付订单失败: {exc}"
            return {"error": self.last_error}

        if response.status_code != 200:
            self.last_error = f"创建支付订单失败，HTTP状态码 {response.status_code}"
            return {"error": self.last_error}
    ...
    def is_paid(self, out_trade_no):
        ...
        try:
            ...
            self.last_error = f"查询支付状态失败: {exc}"
            return False
...
def get_config():
    """动态获取插件配置，兼容共享收款码和全局码支付配置"""
    try:
        pay_config = get_pay_config()
        price = max(
            0,
            parse_float(
                middleware.bucketGet(bucket=BUCKET_CONFIG, key="price"),
                DEFAULT_PRICE,
            ),
        )
        zsm = (
        ...
        points_per_month = max(
            0,
            safe_int(
                middleware.bucketGet(bucket=BUCKET_CONFIG, key="points_per_month"),
        ...
        return {
            ...
            "use_ma_pay": parse_bool(
                middleware.bucketGet(bucket=BUCKET_CONFIG, key="use_ma_pay")
            ),
...
def handle_authorize_payment(target_label, months, account_count=1):
    ...
    ma_pay_enabled = (
        config["use_ma_pay"]
        and config["ma_pay_switch"]
        and config["ma_pay_ready"]
        and config["pay_types"]
    )
    ...
    if ma_pay_enabled:
        ...
        option_lines.append(f"[{next_index}] 码支付 ¥{total_price:.2f}")
        next_index += 1
...
=================="""
    ...
    if selected == "ma":
        return ma_payment_flow(display_name, months, total_price, config)
    if selected == "points":
...
def ma_payment_flow(display_name, months, amount, config):
    ...
    if not client.is_configured():
        sender.reply(f"❌ 码支付配置不完整，请检查卡密系统中的全局码支付配置：{client.last_error}")
        return False

    out_trade_no = f"XMYX{int(time.time())}{str(userid)[-4:]}"
    subject = f"{PROJECT_NAME}授权-{display_name[:18]}"
    order_result = client.create_order(amount, pay_type_key, out_trade_no, subject, str(userid))

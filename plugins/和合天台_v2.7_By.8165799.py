...
# [description: 和合天台代挂提交插件。<br>v2.3修复清理授权功能：新增全局抽奖Q值和免Q值登录开关。2.5修复查询错误问题<br>指令：和合登录、和合管理、和合查询、和合授权<br> 购插件送脚本，脚本在售后群1003974618。📞 售后联系：QQ 8165799<br>]
...
==================""")
        ...
        if config.get('require_q_link', False):
            if len(parts) < 3:
                sender.reply("❌ 格式错误，请按照 手机号#密码#Q值的分享链接 格式输入")
                return
            phone, pwd, q = parts[0], parts[1], parts[2]
            if "q=" not in q:
                sender.reply(f"""
❌ Q值链接错误！
------------------
...
""")
            ...
            if len(parts) < 2:
                sender.reply("❌ 格式错误，请按照 手机号#密码 格式输入")
                return
            phone, pwd = parts[0], parts[1]
            q = ""
        ...
        if login_res.get("code") == 0:
            ...
            process_account_binding(input_str, phone, nick, remark)
        else:
            sender.reply(f"❌ 登录失败: {login_res.get('message', '未知错误')}")

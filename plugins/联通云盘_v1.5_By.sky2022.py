# [rule: ^联通云盘(登录|登陆|查询|管理|授权|后台|教程)$]
# [disable:false]
...
# [author: sky2022]
# [service: 2661320550]
# [description: V1.5:此版本更新适配了呆呆面板丨联通云盘抽奖和联通权益插件，支持验证码/账密登录<br>指令：联通云盘登录、联通云盘查询、联通云盘管理、联通云盘授权、联通云盘后台、联通云盘教程]
...
# [param: {"required":true,"key":"dd_ltyp.ql_config","bool":false,"placeholder":"Host丨ClientID丨ClientSecret","name":"设置对接容器","desc":"青龙配置,用丨分割"}]
# [param: {"required":true,"key":"dd_ltyp.var_name","bool":false,"placeholder":"必填项,例:LTYPCookie","name":"青龙变量名","desc":"提交到青龙的变量名"}]
# [param: {"required":true,"key":"dd_ltyp.ql_format","bool":false,"placeholder":"1或2或3","name":"青龙变量格式","desc":"1=手机号#token_online 2=token_online 3=手机号#ecs_token"}]
...
# [param: {"required":false,"key":"dd_ltyp.use_daidai","bool":true,"placeholder":"false","name":"使用呆呆面板","desc":"是否使用呆呆面板管理变量（开启后将使用呆呆面板替代青龙）"}]
# [param: {"required":false,"key":"dd_ltyp.dd_ltyp_ddname","bool":false,"placeholder":"http://面板地址丨AppKey丨AppSecret","name":"呆呆面板配置","desc":"呆呆面板配置,用丨分割: 面板地址丨AppKey丨AppSecret"}]


def get_config():
    """获取插件配置"""
    var_name = middleware.bucketGet('dd_ltyp', 'var_name') or 'LTYPCookie'
    ql_config = middleware.bucketGet('dd_ltyp', 'ql_config') or ''
    ql_format = middleware.bucketGet('dd_ltyp', 'ql_format') or '1'
    return var_name, ql_config, ql_format

def get_daidai_config():
    """获取呆呆面板配置"""
    use_daidai = middleware.bucketGet('dd_ltyp', 'use_daidai') or 'false'
    use_daidai = use_daidai.lower() == 'true'
    dd_ltyp_ddname = middleware.bucketGet('dd_ltyp', 'dd_ltyp_ddname') or ''
...
def _save_sms_account(phone, token_online, ecs_token):
    ...
    # 保存到用户账号列表
    if not uservalue:
        middleware.bucketSet('dd_ltyp_user', userid, str([phone]))
    else:
...
def process_payment(project, me_as_int, account_vip, token, phone, account, money, vip_money, vip_coin, ecs_token=None):
    ...
    if money == 0:
        ...
        if ql_url and ql_token and token:
            add_to_qinglong(ql_url, ql_token, var_name, token, phone, '', ql_format, ecs_token, new_vip)
    ...
    if total_money == 0:
        ...
        for account in accounts:
            try:
                ...
                ql_url, ql_token, var_name, ql_format = init_qinglong()
                if ql_url and ql_token and token:
                    add_to_qinglong(ql_url, ql_token, var_name, token, account, '', ql_format, ecs_token, new_vip)
                success_count += 1
            except:
                continue
        ...
        for user in users:
            ...
            for account in accounts:
                try:
                    ...
                    ql_url, ql_token, var_name, ql_format = init_qinglong()
                    if ql_url and ql_token:
                        add_to_qinglong(ql_url, ql_token, var_name, token, account, '', ql_format, ecs_token, new_sqsj)
                    success_count += 1
                except:
                    fail_count += 1

# [rule: ^(广汽)(登录|登陆)$|^登(录|陆)(广汽)$|^(广汽)(查询|管理)$|^(查询|管理)(广汽)$|^广汽清理$|^广汽授权$|^广汽教程$|^广汽通知 ?(.*)$|^清理广汽$|^广汽广播 ?(.*)$]
# [disable:false]
...
# [description: 广汽代挂提交<br>1. 指令：广汽登录、广汽管理、广汽查询、广汽授权<br>2. 用户提交 手机号#AT#RT，环境变量只写 AT#RT，手机号写入备注并脱敏展示。<br>3. 未授权账号不写入面板。<br>4. 售后群1003974618。 售后联系：QQ 8165799<br>]
...
logger = logging.getLogger('gacmotor_plugin')
...
try:
    middleware.bucketSet(bucket='gacmotor_sender', key=userid, value=str(senderID))
    middleware.bucketSet(bucket='gacmotor_imtype', key=userid, value=str(sender.getImtype()))
except:
    pass

# ===================== 插件配置参数 =====================
# [param: {"required":true,"key":"gacmotor.panel_type","bool":false,"placeholder":"qinglong/daidai","name":"对接面板类型","desc":"qinglong=青龙面板 daidai=呆呆面板"}]
# [param: {"required":true,"key":"gacmotor.gacmotor_qlname","bool":false,"placeholder":"Host丨ClientID/AppKey丨Secret","name":"对接系统配置","desc":"青龙:URL丨ID丨Secret 呆呆:URL丨Key丨Secret"}]
# [param: {"required":true,"key":"gacmotor.gacmotor_osname","bool":false,"placeholder":"默认:GacMotor","name":"系统变量名","desc":"系统容器内变量名(默认为GacMotor)"}]
# [param: {"required":false,"key":"gacmotor.epay_alipay","bool":true,"name":"易支付支付宝","desc":"启用易支付支付宝通道收款"}]
# [param: {"required":false,"key":"gacmotor.epay_wxpay","bool":true,"name":"易支付微信","desc":"启用易支付微信通道收款"}]
# [param: {"required":false,"key":"gacmotor.epay_qqpay","bool":true,"name":"易支付QQ","desc":"启用易支付QQ通道收款"}]
# [param: {"required":false,"key":"gacmotor.epay_url","bool":false,"placeholder":"如 http://pay.xxx.com/","name":"易支付网关","desc":"易支付接口网关地址(需带http及结尾/)"}]
# [param: {"required":false,"key":"gacmotor.epay_pid","bool":false,"placeholder":"","name":"易支付商户ID","desc":"易支付的PID"}]
# [param: {"required":false,"key":"gacmotor.epay_key","bool":false,"placeholder":"","name":"易支付商户密钥","desc":"易支付的KEY密钥"}]
# [param: {"required":false,"key":"gacmotor.enable_zsm","bool":true,"name":"个人微信收款","desc":"在支付菜单中增加个人微信收款码方式"}]
# [param: {"required":false,"key":"gacmotor.zsm","bool":false,"placeholder":"http://xxxx.co/xxx.jpg","name":"微信收款码链接","desc":"填写个人微信收款码直链，不填则不开启"}]
# [param: {"required":true,"key":"gacmotor.zsVipmoney","bool":false,"placeholder":"例:0.88,不填为0元","name":"上车价格","desc":"上车价格(单位:元)/月"}]
# [param: {"required":true,"key":"gacmotor.zscoin","bool":false,"placeholder":"不填为关闭积分支付","name":"积分开通","desc":"授权一个月需要多少积分"}]
# [param: {"required":true,"key":"gacmotor.enable_proxy","bool":true,"name":"启用代理","desc":"是否启用代理功能"}]
# [param: {"required":false,"key":"gacmotor.proxy_pool_url","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
# [param: {"required":true,"key":"gacmotor.points_bucket","bool":false,"placeholder":"默认使用dd_sign_points","name":"积分桶名称","desc":"存储用户积分的桶名称"}]
# [param: {"required":true,"key":"gacmotor.enable_remark","bool":true,"name":"启用备注功能","desc":"是否启用账号备注功能"}]
# [param: {"required":true,"key":"gacmotor.reminder_days","bool":false,"placeholder":"例:2","name":"到期提醒天数","desc":"到期前多少天开始发送提醒通知"}]

def getusercontent():
    """获取插件完整配置"""
    panel_type = middleware.bucketGet('gacmotor', 'panel_type') or 'qinglong'
    panel_type = panel_type.lower()
    
    env_qlconfig = middleware.bucketGet('gacmotor', 'gacmotor_qlname') or ''
    env_name = middleware.bucketGet('gacmotor', 'gacmotor_osname') or 'GacMotor'
    
    if not env_qlconfig:
        sender.reply("❌ 配置错误：请在插件配置中填写【对接系统配置】(面板信息)。")
        exit(0)
    
    gacmotor_managecommand = middleware.bucketGet('gacmotor', 'gacmotor_managecommand') or '广汽管理'
    gacmotor_querycommand = middleware.bucketGet('gacmotor', 'gacmotor_querycommand') or '广汽查询'
    gacmotor_signcommand = middleware.bucketGet('gacmotor', 'gacmotor_signcommand') or '广汽登录'
    
    enable_proxy = (middleware.bucketGet('gacmotor', 'enable_proxy') or 'false').lower() == 'true'
    proxy_pool_url = middleware.bucketGet('gacmotor', 'proxy_pool_url') or ''
    points_bucket = middleware.bucketGet('gacmotor', 'points_bucket') or 'dd_sign_points'
    enable_remark = (middleware.bucketGet('gacmotor', 'enable_remark') or 'false').lower() == 'true'
    
    randommanagecommand = gacmotor_managecommand
    randomquerycommand = gacmotor_querycommand
    randomsigncommand = gacmotor_signcommand
    
    zsVipmoney = Decimal(middleware.bucketGet('gacmotor', 'zsVipmoney') or '0')
    zscoin = int(middleware.bucketGet('gacmotor', 'zscoin') or '0')
    reminder_days = int(middleware.bucketGet('gacmotor', 'reminder_days') or '2')
    
    # 个人微信收款配置
    enable_zsm = (middleware.bucketGet('gacmotor', 'enable_zsm') or 'false').lower() == 'true'
    zsm = middleware.bucketGet('gacmotor', 'zsm') or ''
    
    # 易支付配置提取
    epay_url = middleware.bucketGet('gacmotor', 'epay_url') or ''
    epay_pid = middleware.bucketGet('gacmotor', 'epay_pid') or ''
    epay_key = middleware.bucketGet('gacmotor', 'epay_key') or ''
    epay_alipay = (middleware.bucketGet('gacmotor', 'epay_alipay') or 'true').lower() == 'true'
    epay_wxpay = (middleware.bucketGet('gacmotor', 'epay_wxpay') or 'false').lower() == 'true'
    epay_qqpay = (middleware.bucketGet('gacmotor', 'epay_qqpay') or 'false').lower() == 'true'

    return {
        'panel_type': panel_type,
        'env_name': env_name,
        'env_qlconfig': env_qlconfig,
        'gacmotor_managecommand': gacmotor_managecommand,
        'gacmotor_querycommand': gacmotor_querycommand,
        'gacmotor_signcommand': gacmotor_signcommand,
...
def get_owner_user_id(account, fallback_userid=None):
    ...
    try:
        ...
        pass
    try:
        for owner in middleware.bucketAllKeys(bucket='gacmotor_user'):
            try:
...
def send_daily_admin_report(report_data, force_send=False, notify_status=False):
    ...
    if not force_send and middleware.bucketGet('gacmotor_runtime', report_key):
        if notify_status:
    ...
    if framework_sent:
        try:
            middleware.bucketSet('gacmotor_runtime', report_key, "framework")
        except Exception:
            pass
        if notify_status:
...
def batch_verify_account_ck(tasks, max_workers=MAINTENANCE_CK_MAX_WORKERS):
    # 广汽接口偶发波动时容易误判。为避免误推用户，默认不做失效推送。
    return {}

def send_user_notice(user_id, msg, title="广汽通知"):
    try:
        imtype = middleware.bucketGet(bucket='gacmotor_imtype', key=str(user_id)) or sender.getImtype()
    except:
    ...
    targets = []
    try:
        saved_sender = middleware.bucketGet(bucket='gacmotor_sender', key=str(user_id))
        if saved_sender:
...
def safe_send_message(user_id, msg, log_context=""):
    ok, err = send_user_notice(user_id, msg, "广汽提醒")
    if not ok:
        logger.warning(f"消息发送失败 {log_context}: {err}")
    return ok

def parse_panel_gacmotor_remark(remarks):
    try:
    ...
    try:
        ...
        for env in sys_api.get_all_envs():
            if env.get('name') != config['env_name']:
                continue
            stats['scanned'] += 1
            info = parse_panel_gacmotor_remark(env.get('remarks') or env.get('remark') or '')
            if not info:
                continue
            AccountManager.add_account(info['user'], info['account'])
            middleware.bucketSet(bucket='gacmotor_auth', key=info['account'], value=info['auth_date'])
        ...
        logger.warning(f"广汽面板授权回写本地失败: {e}")
    return stats
...
# ===================== 核心逻辑类 (广汽) =====================
GAC_BASE = "https://next.gacmotor.com"
GAC_APP_VERSION = "6.0.42"
GAC_MP_VERSION = "3.1.2"

# 签名常量来源于广汽传祺 APP 抓包脚本，接口级常量不随版本频繁变化。
GAC_APP_ID = "8c4131ff-e326-43ea-b333-decb23936673"
GAC_APP_KEY = "46856407-b211-4a10-9cb2-5a9b94361614"
GAC_API_SIGN_KEY = "a361588rt20dpol"
...
class GacMotorClient:
    def __init__(self, account_str):
        parts = [p.strip() for p in str(account_str or "").split("#") if p.strip()]
        if len(parts) >= 3 and looks_like_mobile(parts[0]):
            self.mobile = parts[0]
            self.at = parts[1]
            self.rt = parts[2]
        else:
            raise ValueError("账号格式错误，应为 手机号#AT#RT")
    ...
    def _get_proxy(self):
        ...
        try:
            ...
            logger.warning(f"广汽获取代理失败: {e}")
        return None

    def _reset_session(self):
        """重建会话，丢弃可能已失效的 keep-alive 连接（保留 cookies）。"""
    ...
    def _format_request_error(exc):
        """把底层网络异常转换为简短可读的提示，避免展示超长的连接池堆栈。"""
            ...
            try:
                ...
                if isinstance(data, dict):
                    if "resultCode" in data and str(data.get("resultCode")) != "0":
                        raise RuntimeError(str(data.get("resultMsg") or data.get("msg") or "接口返回失败"))
                    if "code" in data and str(data.get("code")) not in ["", "0", "0000"]:
                        raise RuntimeError(str(data.get("msg") or data.get("resultMsg") or "接口返回失败"))
                return data
            except RuntimeError as e:
                last_exc = e
                # 仅“接口返回空内容”视为可重试，其余业务错误直接抛出
                ...
                last_exc = e
                # 连接级异常多源于坏代理或失效的 keep-alive 连接，重建会话丢弃陈旧连接
                self._reset_session()
            except Exception as e:
                last_exc = e
    ...
    def _headers(self, method="get"):
        ...
        sig = md5(f"{ts1}{nonce}{GAC_APP_ID}{GAC_APP_KEY}")
        api_sign = md5(f"{ts2}{GAC_API_SIGN_KEY}").upper()
        headers = {
            "Accept": "application/json",
            "appToken": self.at,
            "deviceCode": self.device_code,
            "current-time": str(ts2),
            "deviceId": self.registration_id,
            "version": GAC_APP_VERSION,
            "nonce": str(nonce),
            "token": self.at,
    ...
    def _mp_headers(self):
        return {
            ...
            "token": self.at,
            "version": GAC_MP_VERSION,
            "xweb_xhr": "1",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 "
                          "(KHTML, like Gecko) Chrome/132.0.0.0 Safari/537.36 "
                          "MicroMessenger/7.0.20.1781 MiniProgramEnv/Windows",
            "Referer": "https://servicewechat.com/wx86a1eb5a53a6973b/420/page-frame.html",
            "Accept-Encoding": "gzip, deflate, br",
            "Accept-Language": "zh-CN,zh;q=0.9",
        }

    def app_post(self, path, payload=None):
        body = "" if payload == "" else json.dumps(payload or {}, ensure_ascii=False, separators=(",", ":"))
    ...
    def app_get(self, path):
        return self.request_json("GET", f"{GAC_BASE}{path}", headers=self._headers("get"))

    def mp_post(self, path, payload=None):
        body = json.dumps(payload or {}, ensure_ascii=False, separators=(",", ":"))
        return self.request_json(
            "POST",
            f"{GAC_BASE}/mall{path}",
    ...
    def refresh_token(self):
        if not self.rt:
            return False
        res = self.app_post("/app/app-api/login/refreshAt", {"refreshToken": self.rt})
        if res and str(res.get("resultCode")) == "0":
    ...
    def get_gdou(self):
        try:
            res = self.app_get("/app/app-api/user/getUserGdou")
            if res and str(res.get("resultCode")) == "0":
                self.gdou = str(res.get("data") or "0")
        except Exception:
            self.gdou = ""
        return self.gdou
    ...
    def get_info(self):
        try:
            ...
            if gdou != "":
                msg += f"\n🫘 G豆: {gdou}"
            return True, True, int(gdou or 0), msg
        except Exception as e:
            err = str(e)
            if "登录" in err or "token" in err.lower() or "AT" in err:
    ...
    def check_info(self):
        return {
            "nickname": self.nickname or f"广汽_{mask_mobile(self.mobile)}",
            "phone": self.mobile,
            "acc_key": self.mobile,
            "acc_type": "mobile",
            "aliases": [self.legacy_key()],
            "legacy_key": self.legacy_key(),
            "final_token": f"{self.at}#{self.rt}",
            "mobile": self.mobile,
        }
# ===================== 管理器类 =====================
class RemarkManager:
    ...
    def get_account_remark(user_id, account_id):
        try:
            remark_data = middleware.bucketGet(bucket='gacmotor_remarks', key=f'{user_id}_{account_id}')
    ...
    def set_account_remark(user_id, account_id, remark):
        try:
            ...
            if remark_clean:
                middleware.bucketSet(bucket='gacmotor_remarks', key=f'{user_id}_{account_id}', value=remark_clean)
                return remark_clean
            return ""
        except: return ""
    ...
    def delete_account_remark(user_id, account_id):
        try:
            middleware.bucketDel(bucket='gacmotor_remarks', key=f'{user_id}_{account_id}')
            return True
        except: return False
...
class AccountManager:
    ...
    def get_accounts(user_id):
        try:
            value = middleware.bucketGet(bucket='gacmotor_user', key=str(user_id))
            if not value: return []
    ...
    def add_account(user_id, account):
        try:
            ...
            if account not in accounts:
                ...
                middleware.bucketSet(bucket='gacmotor_user', key=str(user_id), value=str(accounts))
                return True
            return False
        except: return False
        ...
        try:
            ...
            if account in accounts:
                ...
                if accounts:
                    ...
                    middleware.bucketDel(bucket='gacmotor_user', key=str(user_id))
                return True
            return False
        except: return False
    ...
    def update_account_token(account, token):
        try:
            ...
            middleware.bucketSet(bucket='gacmotor_token', key=str(account), value=encrypted_token)
            return True
        except: return False
    ...
    def update_account_mobile(account, mobile):
        try:
            mobile = str(mobile or "").strip()
            if mobile:
                middleware.bucketSet(bucket='gacmotor_mobile', key=str(account), value=mobile)
                return True
            return False
        except: return False
    ...
    def get_account_mobile(account):
        try:
            return middleware.bucketGet(bucket='gacmotor_mobile', key=str(account)) or ""
        except: return ""
    ...
    def update_account_web_token(account, web_token):
        try:
            ...
            if web_token:
                middleware.bucketSet(bucket='gacmotor_web_token', key=str(account), value=encrypt_token(web_token))
                return True
            return False
        except: return False
    ...
    def get_account_web_token(account):
        try:
            enc = middleware.bucketGet(bucket='gacmotor_web_token', key=str(account))
    ...
    def get_token(account):
        try:
            enc = middleware.bucketGet(bucket='gacmotor_token', key=str(account))
    ...
    def get_all_users():
        try:
            users = middleware.bucketAllKeys(bucket='gacmotor_user')
            user_list = []
            for user in users:
    ...
    def migrate_account(user_id, old_account, new_account, new_token, remark=""):
        try:
            ...
            old_vip = middleware.bucketGet(bucket='gacmotor_auth', key=old_account)
            new_vip = middleware.bucketGet(bucket='gacmotor_auth', key=new_account)
            if old_vip and (not new_vip or str(old_vip) > str(new_vip)):
                middleware.bucketSet(bucket='gacmotor_auth', key=new_account, value=old_vip)

            old_bind_date = middleware.bucketGet(bucket='gacmotor_bind_date', key=old_account)
            if old_bind_date and not middleware.bucketGet(bucket='gacmotor_bind_date', key=new_account):
                middleware.bucketSet(bucket='gacmotor_bind_date', key=new_account, value=old_bind_date)
            ...
            middleware.bucketSet(bucket='gacmotor_user', key=str(user_id), value=str(new_accounts))

            AccountManager.update_account_token(new_account, new_token)
            try: middleware.bucketDel(bucket='gacmotor_token', key=old_account)
            except: pass
            try: middleware.bucketDel(bucket='gacmotor_mobile', key=old_account)
            except: pass
            try: middleware.bucketDel(bucket='gacmotor_web_token', key=old_account)
            except: pass
            try: middleware.bucketDel(bucket='gacmotor_auth', key=old_account)
            except: pass
            return True
        except Exception as e:
...
class SystemAPI:
    ...
    def sync_env(self, token, phone, remark="", auth_time="", owner_user_id=None):
        ...
        try:
            ...
            # 青龙多号模式：每个账号单独一条同名环境变量，value 只保存 token。
            ql_value = f"{token}"
            
            safe_phone = mask_mobile(phone)
            remarks_parts = [f'广汽:{safe_phone}']
            ...
            remarks_parts.extend([f'用户:{owner_user}', f'ID:{phone}', '广汽提交'])
...
def process_single_account_query(account, index, total_count, account_remarks):
    try:
        ...
        accountVip = middleware.bucketGet(bucket='gacmotor_auth', key=account)
        ...
        if full_token and len(full_token) >= 10:
            try:
                client = GacMotorClient(f"{account}#{full_token}")
                ...
                if getattr(client, "refreshed", False):
                    AccountManager.update_account_token(account, f"{client.at}#{client.rt}")
...
==================""")
        ...
        for i, acc in enumerate(accounts, 1):
            ...
            vip = middleware.bucketGet(bucket='gacmotor_auth', key=acc)
            if not vip:
                vip_tag = '⚠️未授权'
            elif vip < today_time:
                vip_tag = '❌已过期'
        ...
        # 广汽服务器偶发掐断连接，控制并发数避免瞬时连接过多触发服务端拒绝
...
def notify_authorized_users():
    ...
    content = ""
    match = re.search(r'^\s*(广汽广播|广汽通知)\s*(.*)$', usermessage, re.S)
    if match:
        content = match.group(2).strip()
    
    if not content:
        sender.reply("❌ 请输入通知内容，例如：广汽通知 系统维护中")
        return
    ...
    try:
        ...
        for uid in all_users:
            ...
            for acc in user_accounts:
                vip_date = middleware.bucketGet(bucket='gacmotor_auth', key=str(acc))
                if vip_date and vip_date >= today:
                    has_auth = True
                    break
            
            if has_auth:
                ok, err = send_user_notice(uid, f"📢 【广汽管理员通知】\n\n{content}", "广汽管理员通知")
                if ok:
                    success_count += 1
                    time.sleep(0.3)
                else:
                    fail_count += 1
                    logger.warning(f"广汽管理员通知发送失败 {uid}: {err}")
...
def is_cancel_input(value):
    try:
        ...
        return text in ['q', 'quit', 'exit', '退出', '取消', 'cancel']
    except:
        return False
...
def parse_bind_input_line(line, default_remark="", submit_mode="password"):
    ...
    if "#" in raw and "token=" not in raw:
        ...
        if len(parts) >= 3 and looks_like_mobile(parts[0]):
            return GacMotorClient("#".join(parts[:3])), default_remark
...
def process_account_binding(full_token, unique_id, nickname, remark="", aliases=None, acc_type="", legacy_key="", mobile="", web_token="", silent=False):
    try:
        ...
        if is_new:
            try: middleware.bucketSet(bucket='gacmotor_bind_date', key=account, value=str(datetime.now().date()))
            except: pass
        ...
        if choice == '1':
            ...
            if process_payment(months, accountVip, token, account, remark):
                try:
                    ...
                    try: middleware.bucketSet(bucket='gacmotor_auth', key=account, value=new_auth_time)
                    except: pass

                    today_date = datetime.now().date()
                    for d in range(config['reminder_days'] + 1):
                        remind_key = f"{userid}_{account}_{today_date - timedelta(days=d)}"
                        try: middleware.bucketDel('gacmotor_remind_log', remind_key)
                        except: pass
            ...
            if get_user_input() == 'y':
                try:
                    ...
                    try: middleware.bucketDel(bucket='gacmotor_token', key=account)
                    except: pass
                    try: middleware.bucketDel(bucket='gacmotor_mobile', key=account)
                    except: pass
                    try: middleware.bucketDel(bucket='gacmotor_web_token', key=account)
                    except: pass
                    try: middleware.bucketDel(bucket='gacmotor_auth', key=account)
                    except: pass
...
def process_payment(months, accountVip, token, account, remark=""):
    ...
    try:
        ...
        if opt['type'] == 'epay':
            out_trade_no = f"SLD_{int(time.time())}_{userid}_{random.randint(1000,9999)}"
...
def batch_auth_selected(accounts, account_remarks):
    ...
    try:
        ...
        if opt['type'] == 'epay':
            out_trade_no = f"SLD_BATCH_{int(time.time())}_{userid}_{random.randint(1000,9999)}"
            formatted_money = f"{float(opt['amount']):.2f}"
            channel_name = "支付宝" if opt['channel'] == 'alipay' else ("微信支付" if opt['channel'] == 'wxpay' else "QQ钱包")
    ...
    for account in accounts:
        try:
            ...
            try: middleware.bucketSet('gacmotor_auth', account, new_date)
            except: pass
...
def clean_expired_accounts(force_report=False):
    ...
    users = middleware.bucketAllKeys(bucket='gacmotor_user')
    if not users:
        if sender.isAdmin() and (force_report or usermessage in ['广汽清理', '清理广汽']):
            sender.reply("=====执行结果=====\n📭 暂无用户数据")
        return {
        ...
        try:
            ...
            for account_item in context["accounts"]:
                ...
                if days_diff >= 0:
                    ...
                    # 广汽不做 CK/配置失效提醒，避免接口不稳定导致误推。
                    is_ck_valid = True
                    
                    if is_ck_valid and 0 <= days_diff <= reminder_days_cfg:
                        remind_key = f"{user}_{account}_{today_date}"
                        has_reminded = middleware.bucketGet('gacmotor_remind_log', remind_key)
...
=================="""
                            if safe_send_message(user, msg, f"到期提醒 {user}-{account}"):
                                try: middleware.bucketSet('gacmotor_remind_log', remind_key, "1")
                                except: pass
                                reminded_count += 1
                    continue
...
=================="""
            ...
            if user_has_change:
                if valid_accounts:
                    try: middleware.bucketSet(bucket='gacmotor_user', key=str(user), value=str(valid_accounts))
                    except: pass
                else:
                    try: middleware.bucketDel(bucket='gacmotor_user', key=str(user))
                    except: pass
    ...
    for user in users:
        for account in AccountManager.get_accounts(user):
            try:
                ...
                if not token:
                    stats["no_token"] += 1
                vip = middleware.bucketGet(bucket='gacmotor_auth', key=account)
                if not vip:
...
def admin_user_ck_preview():
    ...
    for i, row in enumerate(rows, 1):
        ...
        if row["no_token"]:
            extra.append(f"缺TOKEN{row['no_token']}")
    ...
    if choice == '1':
        ...
        if len(users) <= 1 and total_accounts > 5:
            sender.reply(
                ...
                "请先恢复 gacmotor_user 归属表，或使用 [2] 同步指定用户账号。"
            )
            return
...
def admin_auth_all_users():
    ...
    for user in all_users:
        ...
        for account in accounts:
            try:
                account = str(account)
                accVip = middleware.bucketGet(bucket='gacmotor_auth', key=account)
                new_vip = empower(accVip, days)
                try: middleware.bucketSet(bucket='gacmotor_auth', key=account, value=new_vip)
                except: pass
...
def admin_auth_specific_user():
    ...
    for i, acc in enumerate(target_accounts, 1):
        acc = str(acc)
        accVip = middleware.bucketGet(bucket='gacmotor_auth', key=acc)
    ...
    for acc in selected_accs:
        ...
        accVip = middleware.bucketGet(bucket='gacmotor_auth', key=acc)
        new_vip = empower(accVip, days)
        try: middleware.bucketSet(bucket='gacmotor_auth', key=acc, value=new_vip)
        except: pass
...
try:
    ...
    elif re.match(r'^广汽(通知|广播)\s*', command):
        notify_authorized_users()
    elif command in ['广汽登录', '广汽登陆', '登录广汽', '登陆广汽']:
        bindaccount()
    elif command in ['广汽管理', '管理广汽']:
       xy_manage()
    elif command in ['广汽查询', '查询广汽']:
        cxs()
    elif command in ['广汽清理', '清理广汽']:
        try:
            report_data = clean_expired_accounts()
        except Exception:

...
# [rule: ^游侠(.*)|(.*)游侠$] 匹配规则，多个规则时向下依次写多个
# [cron: 8 8 * * *] cron定时，支持5位域和6位域
# [priority: 55] 优先级，数字越大表示优先级越高
# [platform: all] 适用的平台
# [open_source: false]是否开源
# [icon: https://bbs.autman.cn/assets/files/2025-06-20/1750410377-465804-256-13.webp]图标链接地址，请使用48像素的正方形图标，支持http和https
# [version: 1.3.0]版本号
# [public: true] 是否发布？值为true或false，不设置则上传aut云时会自动设置为true，false时上传后不显示在市场中，但是搜索能搜索到，方便开发者测试
# [price: 18.88] 上架价格
# [description: AI练手，账密登录（支持批量）。无内置本，无任何本提供，仅登录查询授权功能，青龙环境变量格式为“账号#密码”<br>查询调用缓存token,暂不知多久过期，后续有反馈再添加刷新token<br>指令：游侠（登录|查询|管理|授权|清理|教程）<br>1.3.0更新：查询报错请更新autman的go环境；<br>1.2.0更新：修复查询ssl报错；<br>1.1.0更新：增加可提现通知；<br>1.0.0初版：支持批量登录。] 使用方法尽量写具体
...
# [param: {"required":true,"key":"mrconli.youxia.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"|"}]
# [param: {"required":false,"key":"mrconli.youxia.var_name","bool":false,"placeholder":"m_yxw","name":"环境变量名","desc":"青龙容器内的变量名，默认为：m_yxw"}]
# [param: {"required":false,"key":"mrconli.youxia.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.youxia.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.youxia.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]
# [param: {"required":false,"key":"mrconli.youxia.is_proxy","bool":true,"placeholder":"","name":"是否启用代理","desc":"true/false"}]
# [param: {"required":false,"key":"mrconli.youxia.proxy_pool","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
...
uservalue = middleware.bucketGet(bucket='mrconli.youxia.user', key=userid)
...
IS_PROXY = middleware.bucketGet('mrconli.youxia', 'is_proxy')  # 是否启用代理True
PROXY_API = middleware.bucketGet('mrconli.youxia', 'proxy_pool')
...
def login(username, passwd):
    ...
    signature_str = "username-" + username + "-time-" + str(time10) + "-passwd-" + passwd + "-from-feedearn-action-loginBGg)K6ng4?&x9sCIuO%C2%{@TJ?fnFJ,bZKy/[/EWnw9UsC$@1"
    ...
    body = f"action=login&username={username}&passwd={passwd}&time={time10}&from=feedearn&signature={signature}"
...
def get_user_info(token):
    url = f"https://api3.ali213.net/feedearn/userbaseinfo?token={token}"
    headers = {
    ...
    response = _send_request('POST', url, headers=headers)
    if response:
        try:
            result = response.json()
        #    sender.reply(f"【查询金币信息】{result}")
            mobile = str(result["mobile"])
            nickname = result["nickname"]
            total = float(result["total"])/100   # 当前余额
            available = float(result["available"])/100  # 可提现余额
            coins = result.get("coins", 0)   # 当前金币
            history = float(result["history"])/100  # 历史累计
            return mobile, nickname, total, coins, history, available
        except Exception as e:
            print(e)
...
def batch_login():
    ...
    for index, account in enumerate(accounts, 1):
        try:
            username, passwd = account.split('#')
            phone, nickname, token = login(username, passwd)
            # 新增格式校验
            if not phone:
                fail_count += 1
                error_reasons.append(f"登录效验失败...")
                continue
            if phone:
                success_count += 1
                middleware.bucketSet('mrconli.youxia.token', phone, token)
                middleware.bucketSet('mrconli.youxia.bind', phone, account)
                current_accounts = eval(middleware.bucketGet('mrconli.youxia.user', userid) or '[]')
                if phone not in current_accounts:
                    add_count += 1
                    status = "✅ 登录成功"
                    current_accounts.append(phone)
                    middleware.bucketSet('mrconli.youxia.user', userid, json.dumps(current_accounts, ensure_ascii=False))
                else:
                    update_count += 1
                    status = "✅ 更新成功"
                    accountVip = middleware.bucketGet('mrconli.youxia.auth', phone)
...
def query():
    ...
    #    sender.reply('正在查询游侠，请耐心等待...')

    for account in target_accounts:
        try:
            accountVip = middleware.bucketGet('mrconli.youxia.auth', account)
            bind = middleware.bucketGet('mrconli.youxia.bind', account)
            username, passwd = bind.split('#')
            phone1, nickname1, ytoken = login(username, passwd)
            time.sleep(1)
        #    token = middleware.bucketGet('mrconli.youxia.token', account)
            if not ytoken:
            ...
            if not accountVip:
                ...
                phone, nickname, total, coins, history, available = get_user_info(ytoken)
                if not phone:
...
def get_config():
    """获取插件配置"""
    try:
        coin_bucket = middleware.bucketGet('mrconli.youxia', 'coin_bucket') or 'dd_sign_points'
        var_name = middleware.bucketGet('mrconli.youxia', 'var_name') or "m_yxw"
        if not var_name:
            print("未配置变量名，使用默认值: m_yxw")
            var_name = 'm_yxw'
            middleware.bucketSet('mrconli.youxia', 'var_name', var_name)
        ql_config = middleware.bucketGet('mrconli.youxia', 'ql_config')
        if not ql_config:
        ...
        if len(ql_params) == 3:
            ...
            print("青龙配置不完整，请检查配置")
        manage_cmd = middleware.bucketGet('mrconli.youxia', 'manage_cmd') or '游侠管理'
        query_cmd = middleware.bucketGet('mrconli.youxia', 'query_cmd') or '游侠查询'
        login_cmd = middleware.bucketGet('mrconli.youxia', 'login_cmd') or '游侠登录'
        try:
            price = Decimal(middleware.bucketGet('mrconli.youxia', 'price') or '1')
            if price < 0:
                raise ValueError("价格不能为负数")
        except (ValueError, decimal.InvalidOperation):
            print("价格配置无效，使用默认值: 1")
            price = Decimal('1')
            middleware.bucketSet('mrconli.youxia', 'price', '1')
        try:
            coin_price = int(middleware.bucketGet('mrconli.youxia', 'coin') or '0')
            if coin_price < 0:
                raise ValueError("积分不能为负数")
        except ValueError:
            print("积分配置无效，使用默认值: 0")
            coin_price = 0
            middleware.bucketSet('mrconli.youxia', 'coin', '0')
        try:
            show_records = int(middleware.bucketGet('mrconli.youxia', 'show_records') or '3')
            if show_records < 1:
                raise ValueError("显示记录数不能小于1")
        except ValueError:
            print("显示记录数配置无效，使用默认值: 3")
            show_records = 3
            middleware.bucketSet('mrconli.youxia', 'show_records', '3')
...
def add_to_qinglong(token, account, username):
    ...
    try:
        ...
        # 创建新变量（优化请求体格式）
        auth_time = middleware.bucketGet('mrconli.youxia.auth', account) or '未授权'
        data = {
            "name": var_name,
            "value": token,
            "remarks": f"游侠账号:{account}丨用户:{userid}丨授权时间:{auth_time}",
        }
        ...
        for attempt in range(max_retries):
            ...
            if response.status_code == 200:
                ...
                middleware.bucketSet('mrconli.youxia.env_id', account, json.dumps(new_ids))
                return True
...
账号列表:"""
    for i, account in enumerate(accounts, 1):
        token = middleware.bucketGet('mrconli.youxia.token', account)
        auth = middleware.bucketGet('mrconli.youxia.auth', account)
    ...
    try:
        if choice == '01':
            ...
            middleware.bucketSet('mrconli.youxia.user', userid, '[]')
            ...
            # 新增配置获取（修复积分显示问题）
            coin_bucket = middleware.bucketGet('mrconli.youxia', 'coin_bucket') or 'dd_sign_points'
            coin_price = int(middleware.bucketGet('mrconli.youxia', 'coin') or '0')  # 确保获取最新积分价格
                ...
                if pay_choice == '1':
                    ...
                    if process_payment(amount, days):
                        ...
                        for account in accounts:
                            ...
                            middleware.bucketSet('mrconli.youxia.auth', account, auth_time)
                            token = middleware.bucketGet('mrconli.youxia.bind', account)
                            username = account
...
====================""")
                ...
                # 更新青龙状态
                for account in accounts:
                    env_id_str = middleware.bucketGet('mrconli.youxia.env_id', account)
                    if env_id_str:
...
def clean_expired():
    ...
    if not sender.isAdmin():
        ...
        return
    users = middleware.bucketAllKeys('mrconli.youxia.user')
    cleaned = 0
    for user in users:
        accounts = eval(middleware.bucketGet('mrconli.youxia.user', user) or '[]')
        valid = []
        for account in accounts:
            auth = middleware.bucketGet('mrconli.youxia.auth', account)
            if not auth or auth <= str(datetime.now().date()):
                middleware.bucketDel('mrconli.youxia.token', account)
                middleware.bucketDel('mrconli.youxia.auth', account)
                middleware.bucketDel('mrconli.youxia.env_id', account)
                cleaned += 1
            else:
                valid.append(account)
        if valid:
            middleware.bucketSet('mrconli.youxia.user', user, str(valid))
        else:
            middleware.bucketDel('mrconli.youxia.user', user)
            ...
            for account in accounts:
                try:
                    ...
                    if auth and auth <= today:
                        ...
                        continue
                    username, passwd = bind.split('#')
                    _, _, token = login(username, passwd)
                    _, _, _, _, _, available = get_user_info(token)
                    if available >= 20:
                        notify_user(user, account, "余额超20，已可提现！")
                except Exception as e:
...
def log_operation(operation, user, account, status, message=''):
    ...
    try:
        ...
        logs = eval(middleware.bucketGet('mrconli.youxia.logs', 'operations') or '[]')
        logs.append(log)
        if len(logs) > 1000:  # 只保留最近1000条
            logs = logs[-1000:]
        middleware.bucketSet('mrconli.youxia.logs', 'operations', str(logs))
    except Exception as e:
    ...
    for user in users:
        ...
        for account in accounts:
            ...
            try:   
                ...
                env_ids_str = middleware.bucketGet('mrconli.youxia.env_id', account)
                if env_ids_str:
...
回复"q"退出""")
    ...
    if not user_id or user_id == 'q':
        return
    accounts = eval(middleware.bucketGet('mrconli.youxia.user', user_id) or '[]')
    if not accounts:
...
def delete_account(account):
    ...
    try:
        ...
        middleware.bucketDel('mrconli.youxia.auth', account)
        middleware.bucketDel('mrconli.youxia.env_id', account)
        middleware.bucketDel('mrconli.youxia.bind', account)
        ...
        if account in accounts:
            ...
            try:
                middleware.bucketSet('mrconli.youxia.user', userid, json.dumps(accounts, ensure_ascii=False))
            except Exception as e:
...
def main():
    ...
    if '登录' in message or '登陆' in message or '上车' in message:
        ...
        clean_expired()
    elif message == '游侠授权' and sender.isAdmin():
        admin_auth()

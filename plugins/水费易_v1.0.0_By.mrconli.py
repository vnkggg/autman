...
# [rule: ^水费易(.*)|(.*)水费易$] 匹配规则，多个规则时向下依次写多个
# [cron: 18 6 * * *] cron定时，支持5位域和6位域
# [priority: 55] 优先级，数字越大表示优先级越高
# [platform: all] 适用的平台
# [open_source: false]是否开源
# [icon: https://bbs.autman.cn/assets/files/2025-06-17/1750161978-465465-4ae76c6a2d62dc654a1dc96f3bbb3256.webp]图标链接地址，请使用48像素的正方形图标，支持http和https
# [version: 1.0.0]版本号
# [public: true] 是否发布？值为true或false，不设置则上传aut云时会自动设置为true，false时上传后不显示在市场中，但是搜索能搜索到，方便开发者测试
# [price: 8.88] 上架价格
# [description: AI练手，微信服务号“水费易”，积分换实物，无需抓包，内置签到本，其他任务请自行找本提交青龙运行。青龙环境变量格式为“会员ID”<br>指令：水费易（登录|查询|管理|签到|一键签到|授权|清理|教程）<br>1.支持积分支付数据桶自定义，不再为积分系统不统一烦恼！<br>2.默认支持调用linzixuan最新积分。<br>1.0.0初版：支持批量登录。] 使用方法尽量写具体
...
# [param: {"required":true,"key":"mrconli.shuifeiyi.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"|"}]
# [param: {"required":false,"key":"mrconli.shuifeiyi.var_name","bool":false,"placeholder":"m_dnys","name":"环境变量名","desc":"青龙容器内的变量名，默认为：m_dnys"}]
# [param: {"required":false,"key":"mrconli.shuifeiyi.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.shuifeiyi.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.shuifeiyi.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]
# [param: {"required":false,"key":"mrconli.shuifeiyi.is_proxy","bool":true,"placeholder":"false","name":"是否启用代理","desc":"true/false"}]
# [param: {"required":false,"key":"mrconli.shuifeiyi.proxy_pool","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
...
uservalue = middleware.bucketGet(bucket='mrconli.shuifeiyi.user', key=userid)
today_date = datetime.now().date()
today_time = str(today_date)
number = int(middleware.bucketGet('mrconli.shuifeiyi', 'number') or 5)

# 代理配置
MAX_RETRIES = 10  # 最大重试次数
IS_PROXY = middleware.bucketGet('mrconli.shuifeiyi', 'is_proxy')  # 是否启用代理True
PROXY_API = middleware.bucketGet('mrconli.shuifeiyi', 'proxy_pool') or 'https://api.mrconli.com/getProxy'
if not PROXY_API:
...
def batch_login():
    ...
    for index, account in enumerate(accounts, 1):
        try:  
            ...
            if phone:
                success_count += 1
                middleware.bucketSet('mrconli.shuifeiyi.token', phone, account)
                current_accounts = eval(middleware.bucketGet('mrconli.shuifeiyi.user', userid) or '[]')
                if phone not in current_accounts:
                    add_count += 1
                    status = "✅ 登录成功..."
                    current_accounts.append(phone)
                    middleware.bucketSet('mrconli.shuifeiyi.user', userid, json.dumps(current_accounts, ensure_ascii=False))
                else:
                    update_count += 1
                    status = "✅ 更新成功..."
                    accountVip = middleware.bucketGet('mrconli.shuifeiyi.auth', phone)
...
def query():
    ...
    for account in target_accounts:
        try:
            accountVip = middleware.bucketGet('mrconli.shuifeiyi.auth', account)
            token = middleware.bucketGet('mrconli.shuifeiyi.token', account)
            if not token:
...
def all_qd():
    """所有用户签到功能"""
    try:
        users = middleware.bucketAllKeys('mrconli.shuifeiyi.user')
        for user in users:
            try:
                accounts = json.loads(middleware.bucketGet('mrconli.shuifeiyi.user', user)) if middleware.bucketGet('mrconli.shuifeiyi.user', user) else []
...
def get_config():
    """获取插件配置"""
    try:
        coin_bucket = middleware.bucketGet('mrconli.shuifeiyi', 'coin_bucket') or 'dd_sign_points'
        var_name = middleware.bucketGet('mrconli.shuifeiyi', 'var_name') or "m_dnys"
        if not var_name:
            print("未配置变量名，使用默认值: m_dnys")
            var_name = 'm_dnys'
            middleware.bucketSet('mrconli.shuifeiyi', 'var_name', var_name)
        ql_config = middleware.bucketGet('mrconli.shuifeiyi', 'ql_config')
        if not ql_config:
        ...
        if len(ql_params) == 3:
            ...
            print("青龙配置不完整，请检查配置")
        manage_cmd = middleware.bucketGet('mrconli.shuifeiyi', 'manage_cmd') or '水费易管理'
        query_cmd = middleware.bucketGet('mrconli.shuifeiyi', 'query_cmd') or '水费易查询'
        login_cmd = middleware.bucketGet('mrconli.shuifeiyi', 'login_cmd') or '水费易登录'
        try:
            price = Decimal(middleware.bucketGet('mrconli.shuifeiyi', 'price') or '1')
            if price < 0:
                raise ValueError("价格不能为负数")
        except (ValueError, decimal.InvalidOperation):
            print("价格配置无效，使用默认值: 1")
            price = Decimal('1')
            middleware.bucketSet('mrconli.shuifeiyi', 'price', '1')
        try:
            coin_price = int(middleware.bucketGet('mrconli.shuifeiyi', 'coin') or '0')
            if coin_price < 0:
                raise ValueError("积分不能为负数")
        except ValueError:
            print("积分配置无效，使用默认值: 0")
            coin_price = 0
            middleware.bucketSet('mrconli.shuifeiyi', 'coin', '0')
        try:
            show_records = int(middleware.bucketGet('mrconli.shuifeiyi', 'show_records') or '3')
            if show_records < 1:
                raise ValueError("显示记录数不能小于1")
        except ValueError:
            print("显示记录数配置无效，使用默认值: 3")
            show_records = 3
            middleware.bucketSet('mrconli.shuifeiyi', 'show_records', '3')
...
def add_to_qinglong(token, account, username):
    ...
    try:
        ...
        # 创建新变量（优化请求体格式）
        auth_time = middleware.bucketGet('mrconli.shuifeiyi.auth', account) or '未授权'
        data = {
            "name": var_name,
            "value": token,
        ...
        for attempt in range(max_retries):
            ...
            if response.status_code == 200:
                ...
                middleware.bucketSet('mrconli.shuifeiyi.env_id', account, json.dumps(new_ids))
                return True
...
账号列表:"""
    for i, account in enumerate(accounts, 1):
        token = middleware.bucketGet('mrconli.shuifeiyi.token', account)
        auth = middleware.bucketGet('mrconli.shuifeiyi.auth', account)
    ...
    try:
        if choice == '01':
            ...
            middleware.bucketSet('mrconli.shuifeiyi.user', userid, '[]')
            ...
            # 新增配置获取（修复积分显示问题）
            coin_bucket = middleware.bucketGet('mrconli.shuifeiyi', 'coin_bucket') or 'dd_sign_points'
            coin_price = int(middleware.bucketGet('mrconli.shuifeiyi', 'coin') or '0')  # 确保获取最新积分价格
                ...
                if pay_choice == '1':
                    ...
                    if process_payment(amount, days):
                        ...
                        for account in accounts:
                            ...
                            middleware.bucketSet('mrconli.shuifeiyi.auth', account, auth_time)
                            token = middleware.bucketGet('mrconli.shuifeiyi.token', account)
                            username = account
...
====================""")
                ...
                # 更新青龙状态
                for account in accounts:
                    env_id_str = middleware.bucketGet('mrconli.shuifeiyi.env_id', account)
                    if env_id_str:
...
def clean_expired():
    ...
    cleaned = 0
    for user in users:
        accounts = eval(middleware.bucketGet('mrconli.shuifeiyi.user', user) or '[]')
        valid = []
        for account in accounts:
            auth = middleware.bucketGet('mrconli.shuifeiyi.auth', account)
            if not auth or auth <= str(datetime.now().date()):
                middleware.bucketDel('mrconli.shuifeiyi.token', account)
                middleware.bucketDel('mrconli.shuifeiyi.auth', account)
                middleware.bucketDel('mrconli.shuifeiyi.env_id', account)
                cleaned += 1
            else:
                valid.append(account)
        if valid:
            middleware.bucketSet('mrconli.shuifeiyi.user', user, str(valid))
        else:
            middleware.bucketDel('mrconli.shuifeiyi.user', user)
...
def log_operation(operation, user, account, status, message=''):
    ...
    try:
        ...
        logs = eval(middleware.bucketGet('mrconli.shuifeiyi.logs', 'operations') or '[]')
        logs.append(log)
        if len(logs) > 1000:  # 只保留最近1000条
            logs = logs[-1000:]
        middleware.bucketSet('mrconli.shuifeiyi.logs', 'operations', str(logs))
    except Exception as e:
    ...
    for user in users:
        ...
        for account in accounts:
            ...
            try:   
                ...
                env_ids_str = middleware.bucketGet('mrconli.shuifeiyi.env_id', account)
                if env_ids_str:
...
回复"q"退出""")
    ...
    if not user_id or user_id == 'q':
        return
    accounts = eval(middleware.bucketGet('mrconli.shuifeiyi.user', user_id) or '[]')
    if not accounts:
...
def delete_account(account):
    ...
    try:
        ...
        if account in accounts:
            ...
            try:
                middleware.bucketSet('mrconli.shuifeiyi.user', userid, json.dumps(accounts, ensure_ascii=False))
            except Exception as e:
...
def tutorial():
    """显示水费易使用教程"""
    tutorial_text = (
        "[CQ:image,file=https://bbs.autman.cn/assets/files/2025-06-17/1750162614-990316-567139274dae9a67a5e369f51a18fcda.webp]"
        "=====水费易教程=====\n"
        "📝 入口:\n"
        "    微信搜索服务号：水费易\n"

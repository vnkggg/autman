...
#[language: python]
#[class: 工具类]
#[service: linzixuan] 售后联系方式
#[author: linzixuan] 作者
#[disable: false] 禁用开关，true表示禁用，false表示可用
#[admin: false] 是否为管理员指令
#[rule: ^小米登录$|^登录小米$|^小米查询$|^小米管理$|^小米清理$|^小米一键更新$|^小米授权$|^小米兑换$|^小米教程$] 匹配规则，多个规则时向下依次写多个
...
#[price: 18.88] 上架价格
#[description: 优先购买作者订阅源by：rujingxianghai<br>功能：钱包视频会员(2.5天)|视频会员兑换<br>登录方式：扫码登录<br>指令：小米登录|登录小米|小米查询|小米管理|小米清理|小米一键更新|小米授权|小米兑换|小米教程<br>2.4更新日志：新增手动Cookie登录<br>2.7更新日志：移除抢兑功能，简化兑换流程] 
...
def get_oaid_for_account(account):
	...
	# 格式：user_id#cookie#oaid 或 user_id#cookie
	if len(parts) >= 3:
		...
		middleware.bucketSet('s_xiaomi_token', account, new_cookie_str)
		return oaid
...
def add_to_qinglong(token,account,phone):
	try:
		...
		if len(parts)==3:middleware.bucketSet('s_xiaomi_token',account,new_format)
...
def qr_login(phone,password=None):
	try:
		...
		cookie_str=cookie_to_string(login_data['cookies']);new_cookie_str=f"{login_data['user_id']}#{cookie_str}";middleware.bucketSet('s_xiaomi_token',input_phone,new_cookie_str);result=process_login(login_data['cookies'],input_phone,login_data['user_id'])
...
请输入Cookie:""");cookie_input=sender.listen(120000)
		...
		accounts=eval(uservalue or'[]')
		if input_phone not in accounts:accounts.append(input_phone);middleware.bucketSet('s_xiaomi_user',userid,str(accounts));uservalue=str(accounts)
		cookie_str=cookie_to_string(cookies);new_cookie_str=f"{user_id}#{cookie_str}";middleware.bucketSet('s_xiaomi_token',input_phone,new_cookie_str);result=process_login(cookies,input_phone,user_id)
...
def process_login(cookies,phone,user_id,skip_auth=False,skip_add_account=False):
	try:
		...
		cookie_str=cookie_to_string(cookies);new_cookie_str=f"{user_id}#{cookie_str}";middleware.bucketSet('s_xiaomi_token',phone,new_cookie_str);auth_time=middleware.bucketGet('s_xiaomi_auth',phone);current_date=str(datetime.now().date());is_authorized=auth_time and auth_time>current_date
		if skip_auth:
			if is_authorized:
...
def process_authorization_xiaomi(account,months):
	try:
		...
		if len(parts)==3:cookie_str=parts[1]  # 修复：应该使用parts[1]而不是parts[2]
		elif len(parts)==2:cookie_str=parts[1]
		else:sender.reply(f"❌ 账号 {format_phone(account)} 的Cookie格式错误");return False
		
		# 清理Cookie字符串中可能包含的错误#符号
...
def query_single_account(account):
	try:
		...
		video_days=get_video_days(cookies, account);today_video_days=get_today_video_days(cookies, account);auth_time=middleware.bucketGet('s_xiaomi_auth',account)or'未授权';query_msg=f"""
=====账号信息=====
📱 账号: {format_phone(account)}
👤 UID: {user_info["uid"]}
📅 到期: {auth_time}
	...
	try:
		...
		for user in users:
			...
			for account in accounts:
				...
				if not auth or auth<=today:middleware.bucketSet('s_xiaomi_token',account,'');middleware.bucketSet('s_xiaomi_auth',account,'');delete_from_qinglong(account);cleaned+=1
...
发送"小米查询"查看账号详情''';sender.reply(success_msg)
			...
			for account in selected_accounts:
				try:
					accounts.remove(account);middleware.bucketSet('s_xiaomi_token',account,'');middleware.bucketSet('s_xiaomi_auth',account,'')
...
def exchange_for_account(account,mode='direct'):
	try:
		sender.reply(f"🔄 正在为账号 {format_phone(account)} 获取兑换信息...");		cookie_str=middleware.bucketGet('s_xiaomi_token',account)
...
def update_cookie_by_passtoken(account):
	try:
		...
		# 从parts[1]获取cookie字符串
		old_cookie_str=parts[1]
		
		# 修复：检查并清理Cookie字符串中错误包含的#符号
		# 如果Cookie中包含"userId=xxx#"这样的格式，移除#及其后面的内容
		# 清理userId值中可能包含的#和后续内容
		old_cookie_str = re.sub(r'(userId=\d+)#[^;]*', r'\1', old_cookie_str)
		...
		middleware.bucketSet('s_xiaomi_token',account,updated_cookie_str);return True,'更新成功'

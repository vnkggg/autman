...
# [rule: ^酒仙(.*)$] 匹配规则，多个规则时向下依次写多个
# [cron: 39 7,16 * * *] cron定时，支持5位域和6位域
# [priority: 99999999] 优先级，数字越大表示优先级越高
# [platform: all] 适用的平台
# [open_source: false]是否开源
# [icon: https://pp.myapp.com/ma_icon/0/icon_10072620_1758940657/256]图标链接地址，请使用48像素的正方形图标，支持http和https
# [version: 1.2.0]版本号
# [public: true] 是否发布？值为true或false，不设置则上传aut云时会自动设置为true，false时上传后不显示在市场中，但是搜索能搜索到，方便开发者测试
# [price: 5.88] 上架价格
# [description: AI练手，账密提交青龙。”<br>指令：酒仙（登录|查询|管理|授权|清理|教程）<br>1.0.0初版：支持ck批量登录。] 使用方法尽量写具体
...
# [param: {"required":true,"key":"mrconli.jiuxian.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"http://ip:端口丨ClientID丨ClientSecret"}]
# [param: {"required":false,"key":"mrconli.jiuxian.var_name","bool":false,"placeholder":"JX_TOKENS","name":"环境变量名","desc":"青龙容器内的变量名，默认为：JX_TOKENS"}]
# [param: {"required":false,"key":"mrconli.jiuxian.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.jiuxian.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.jiuxian.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]



scripts_name =  "酒仙"
bucket_prefix = "mrconli.jiuxian"
...
DEVICE_ID = "ad96ade2-b918-3e05-86b8-ba8c34747b0c"   ##   "ad96ade2-b918-3e05-86b8-ba8c34747b0c"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("success") == "1":
                    ...
                    self.nickname = user_info.get("nickName", "未知昵称")
                    return True, self.token, self.nickname
                else:
                    print(f"❌ {self.username}登录失败: {result.get('errMsg', '未知错误')}")
                    return False, None, None
            else:
                print(f"❌ {self.username}登录请求失败: HTTP {response.status_code}")
                return False, None, None
        except Exception as e:
            print(f"❌ {self.username}登录异常: {str(e)}")
            return False, None, None

    def get_member_info(self) -> Optional[Dict]:
        """获取会员信息（包含任务列表和taskToken）"""
        if not self.token:
            print(f"❌ {self.username}请先登录")
            return None  
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get("success") == "1":
                    goldMoney = result["result"].get("goldMoney", 0)  ##  积分
                    isSignTody = result["result"].get("isSignTody", False)  ##  是否签到
                    print(f"🔍 获取会员信息成功 ({self.username}): 积分 {goldMoney}，签到状态 {isSignTody}")
                    return True, goldMoney, isSignTody
                else:
                    return False, None, None, None
            else:
                print(f"❌ 获取会员信息请求失败 ({self.username}): HTTP {response.status_code}")
                return False, None, None, None
        except Exception as e:
            print(f"❌ 获取会员信息异常 ({self.username}): {str(e)}")
            return False, None, None, None
    ...
    for index, account in enumerate(accounts, 1):
        try:
            ...
            if not phone or not password:
                fail_count += 1
                error_reasons.append(f"❌ {account} 账号或密码为空")
                continue
            update_proxy()
            jiuxian = Jiuxian(phone, password)
            success, _, nickname = jiuxian.login_with_password()
            # 新增格式校验
            if not success: 
                fail_count += 1
    ...
    for account in target_accounts:
        try:
            ...
            if not accountVip:
                ...
                jiuxian = Jiuxian(phone, password)
                success, j_token, nickname = jiuxian.login_with_password()
                if not success:
                    sender.reply(f'❌ 【{mask_phone(account)}】查询失败')
                    continue
                success, goldMoney, isSignTody = jiuxian.get_member_info()
                sender.reply(f"""
====={scripts_name}账号详情=====
📱 账号: {mask_phone(phone)}
👤 昵称: {nickname}
🎯 积分: {goldMoney}
📝 今日签到：{"已签到" if isSignTody else "未签到"}
⏰ 授权到期：{accountVip}
==================""")
        except Exception as e:
            ...
            for account in accounts:
                try:
                    ...
                    if auth and auth <= today:
                        ...
                        continue
                    token = middleware.bucketGet(f'{bucket_prefix}.token', account)
                    success, phone, coin = get_userinfo(token)
                    if not success:
                        notify_user(user, account, "token失效，请重新登录")
                        continue
                except Exception as e:
...
def get_config():
    ...
    try:
        ...
        var_name = middleware.bucketGet(bucket_prefix, 'var_name') or "JX_TOKENS"
        if not var_name:
            print("未配置变量名，使用默认值: JX_TOKENS")
            var_name = 'JX_TOKENS'
...
def add_to_qinglong(token, account, username):
    ...
    try:
        ...
        data = {
            "name": var_name,
            "value": token,
            "remarks": f"{scripts_name}账号:{account}丨用户:{userid}丨授权时间:{auth_time}",
        }
...
def tutorial():
    ...
    tutorial_text = (
        ...
        "📝 入口:\n"
        "   APP：酒仙\n"
        "[CQ:image,file=https://bbs.autman.cn/assets/files/2025-11-03/1762166549-993927-jiuxiantg.png]"
        "🌟 基础指令:\n"

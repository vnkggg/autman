...
# [rule: ^潇洒(.*)|(.*)潇洒$] 匹配规则，多个规则时向下依次写多个
# [cron: 48 8,18 * * *] cron定时，支持5位域和6位域
...
# [price: 8.88] 上架价格
# [description: 无脚本提供。ck提交青龙，格式：账号#密码<br>指令：潇洒(登录|登陆|上车|查询|管理|授权|清理)<br>仅环境变量提交青龙<br>支持积分支付授权,可自定义积分数据桶。<br>1.0.0初版：支持授权、批量登录、代理、积分开通等功能] 使用方法尽量写具体
...
# [param: {"required":true,"key":"mrconli.xstl.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"|"}]
# [param: {"required":false,"key":"mrconli.xstl.var_name","bool":false,"placeholder":"xiaosa","name":"环境变量名","desc":"青龙容器内的变量名，默认为：xiaosa"}]
# [param: {"required":false,"key":"mrconli.xstl.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.xstl.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.xstl.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]
# [param: {"required":false,"key":"mrconli.xstl.is_proxy","bool":true,"placeholder":"","name":"是否启用代理","desc":"开启代理就勾选，其实不需要代理"}]
# [param: {"required":false,"key":"mrconli.xstl.proxy_pool","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
...
scripts_name =  "潇洒"
full_scripts_name =  "潇洒桐庐"
bucket_prefix = "mrconli.xstl"
...
def sha256_encrypt(s):
    return hashlib.sha256(s.encode()).hexdigest()

def random_uuid():
    return hex(int(random.random() * 2147483648)).replace("0x", "")

def login_xstl(mobile, password):
    """登录潇洒桐庐，返回 (success, account_id, session_id, nickname, error_msg)"""
    try:
        # 第一步：获取code
        ...
        resp = _send_request('POST', url, headers=headers, data=body)
        data = resp.json()
        if data.get('code') != 0:
            return False, None, None, None, data.get('message', '鉴权失败')
        code = data['data']['authorization_code']['code']
        
        # 第二步：登录获取session
        uuid_str = random_uuid()
        timestamp = int(time.time() * 1000)
        path = "/api/zbtxz/login"
        sign_str = f"{path}&&6565886da95d5a47f651317f&&{uuid_str}&&{timestamp}&&FR*r!isE5W&&59"
        signature = sha256_encrypt(sign_str)
        headers2 = {
        ...
        resp2 = _send_request('POST', "https://vapp.tmuyun.com/api/zbtxz/login", headers=headers2, data=body2)
        data2 = resp2.json()
        if data2.get('code') != 0:
            return False, None, None, None
        account_id = data2['data']['account']['id']
        session_id = data2['data']['session']['id']
        nickname = data2['data']['account']['nick_name']
        return True, account_id, session_id, nickname
    except Exception as e:
        return False, None, None, None

def get_user_info_xstl(account_id, session_id):
    """获取用户积分和最近5次抽奖记录
    返回: (success, point, lottery_lines)
    lottery_lines: 格式化后的最近5条记录字符串
    """
    try:
        # 1. 获取积分
        uuid_str = random_uuid()
        ...
        sign_str = f"{path}&&{session_id}&&{uuid_str}&&{timestamp}&&FR*r!isE5W&&59"
        signature = sha256_encrypt(sign_str)
        headers = {
        ...
        resp = _send_request('GET', "https://vapp.tmuyun.com/api/user_mumber/account_detail?osTypeCode=2", headers=headers)
        data = resp.json()
        if data.get('code') != 0:
            return False, 0, "获取积分失败"
        point = data['data']['rst']['total_integral']

        # 2. 获取最近5次抽奖记录
        lottery_lines = ""
        try:
            # 使用新的抽奖记录接口
            url = f"https://wxapi.hoolo.tv/event/dtqp/index.php?s=home/ChoujiangNew/getUserCj/&openid={account_id}&type_id=122"
            headers_wx = {
                'Host': 'wxapi.hoolo.tv',
            ...
            resp_lot = _send_request('GET', url, headers=headers_wx)
            raw_text = resp_lot.text
            #  抽奖记录原始文本: ({"code":"1","msg":[{"choujiang_recordid":"5058889","name":"","mobile":"","idcard":"","address":"","prize_name":"\u7ea2\u53050.5","code":"batch2026042410002844904","code_image":"131000303038001881878302026042441098130658","remark":"ACCEPTED","code_time":"0","ljdz":"","create_time":"2026-04-24 10:00:28"},{"choujiang_recordid":"5058343","name":"","mobile":"","idcard":"","address":"","prize_name":"\u7ea2\u53050.5","code":"batch2026042312070099818","code_image":"131000003077401881878302026042372137922219","remark":"ACCEPTED","code_time":"1","ljdz":"","create_time":"2026-04-23 12:07:00"}]})
            ...
            records = lot_data.get('msg', [])
            if records:
                # 只取最近5条
                recent_five = records[:5]
                lines = []
                for rec in recent_five:
                    # 假设字段为 create_time 和 prize_name
                    create_time = rec.get('create_time', '')  # 格式: 2026-04-23 09:13:58
                    prize_name = rec.get('prize_name', '谢谢参与')
                    lines.append(f"{create_time}[{prize_name}]")
                lottery_lines = "\n".join(lines)
        except Exception as e:
            lottery_lines = "获取失败"
        return True, point, lottery_lines
    except Exception as e:
        return False, 0, f"获取信息异常: {str(e)}"
    ...
    for index, account in enumerate(accounts, 1):
        try: 
            ...
            if len(parts) == 2:
                ...
                continue
            success, account_id, session_id, nickname = login_xstl(phone, password)
            if not success:
                fail_count += 1
    ...
    for account in target_accounts:
        try:
            ...
            if not accountVip:
                ...
                success, account_id, session_id, nickname = login_xstl(phone, password)
                if not success:
                    sender.reply(f'❌ {mask_phone(account)}：登录认证失败')
                    continue
                success, point, lottery_lines = get_user_info_xstl(account_id, session_id)
                sender.reply(f"""
====={scripts_name}账号详情=====
...
def get_config():
    """获取插件配置"""
    try:
        var_name = middleware.bucketGet(bucket_prefix, 'var_name') or "xiaosa"
        if not var_name:
            print("未配置变量名，使用默认值: xiaosa")
            var_name = 'xiaosa'

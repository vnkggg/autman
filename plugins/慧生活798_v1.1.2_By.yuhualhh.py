...
# [author: yuhualhh]
# [service: ]
# [description: <br>❶慧生活798内置任务插件，自动完成每日任务获取315左右积分，支持Token登录、手机号登录、自定义并发、管理、查询、授权、检测授权过期以及Token失效推送等功能<br>❷部分功能的实现需自行添加计划任务伪装管理员定时，了解如何添加计划任务请看移动云盘插件介绍，关于指令『慧生活检测』与『慧生活清理』定时『30 18 * * *』，关于指令『慧生活一键运行』定时『0 7,19 * * *』<img src="https://gcore.jsdelivr.net/gh/lhz03/img@7b9ff2cf7c406da88575a68400cff2d4604ddaca/2026/04/08/ab67377a6deec9c1303a10353b9d7666.png">]
...
class HSH:
    ...
    def _request_safe(self, method, url, platform="app", data=None, retry=3):
        """
        带风控处理的安全请求封装 (复刻脚本逻辑)
        处理: 频繁冷却、自动重试、平台伪装
        """
        ...
        if platform == "zfb":
            req_headers = {
                ...
                "User-Agent": "Mozilla/5.0 (Linux; Android 10; Mobile; rv:126.0) AlipayClient/10.7.80"
            }
        
        for i in range(retry):
            try:
                # 调用原有的 _send_request (它已包含基础的 Authorization 等头)
                ...
                # 1. 频繁冷却
                if "频繁" in res.text:
                    if DEBUG: printf("⏳ 请求频繁，冷却 30 秒...", "WARN")
                    time.sleep(30)
                    continue 
    ...
    def get_score_history(self):
        """获取积分明细 (用于反推任务进度)"""
        url = "https://i.ilife798.com/api/v1/acc/score/score-lst?page=0&size=50&hasCount=true"
        res = self._send_request('GET', url)
        if not res: return []
        try:
            data = res.json()
    ...
    def get_week_signed_set(self, history):
        ...
        for item in history:
            ...
            if r_date >= monday and r_date <= today:
    ...
    def send_sign_in_request(self, weekday_iso, score, uid, is_retro=False):
        ...
        if not sign:
            return False, "获取签名失败"
        url = f"https://i.ilife798.com/api/v1/acc/score/score-send?sign={sign}"
        ...
        if data and data.get("code") == 0:
            return True, f"成功({score}分)"
        
        msg = data.get("msg") if data else "未知"
        # 降级尝试
        if score > 5:
            if DEBUG: printf(f"⚠️ 高分签到失败({msg})，降级尝试 5 分...", "WARN")
            time.sleep(2)
            return self.send_sign_in_request(weekday_iso, 5, uid, is_retro)
        else:
            return False, msg

    def handle_sign_in_process(self, uid, task_logs):
        """执行智能签到逻辑 (补签 + 今日签到)"""
        history = self.get_score_history()
        signed_days = self.get_week_signed_set(history)
        today_iso = datetime.now().isoweekday()
        has_action = False

        # 1. 补签逻辑
        missed_days = [d for d in range(1, today_iso) if d not in signed_days]
        if missed_days:
        ...
        # 2. 今日签到
        if today_iso not in signed_days:
    ...
    def do_task_logic(self, task_key, current_count, task_config, uid, task_logs):
        ...
        if DEBUG: printf(f"🚀 执行 [{cfg['name']}] (剩余 {remain} 次)", "INFO")
        ...
        for i in range(remain):
            ...
            if task_key == "APP_VIDEO":
                payload.update({"addScore": 30, "addScoreType": 2, "type": 101})
            elif task_key == "APP_AD":
                payload.update({"addScore": 10, "addScoreType": 2, "type": 101})
            else:
    ...
    def run_daily_tasks(self, phone_mask):
        ...
        for round_idx in range(1, 4):
            ...
            # 1. 智能签到处理
            did_sign = self.handle_sign_in_process(uid, task_logs)

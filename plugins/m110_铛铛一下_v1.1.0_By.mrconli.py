...
# [rule: ^铛铛(.*)|(.*)铛铛|快宝(.*)|(.*)快宝$] 匹配规则，多个规则时向下依次写多个
# [cron: 50 8,18 * * *] cron定时，支持5位域和6位域
...
# [price: 18.88] 上架价格
# [description: 支持微信铛铛一下、快宝、支付宝小程序3端，自动识别。无脚本提供。ck提交青龙，格式：token#type#备注<br>指令：铛铛(登录|登陆|上车|查询|管理|授权|清理)<br>仅环境变量提交青龙<br>支持积分支付授权,可自定义积分数据桶。<br>1.0.0初版：支持授权、批量登录、代理、积分开通等功能] 使用方法尽量写具体
...
# [param: {"required":true,"key":"mrconli.ddyx.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"|"}]
# [param: {"required":false,"key":"mrconli.ddyx.var_name","bool":false,"placeholder":"m_ddyx","name":"环境变量名","desc":"青龙容器内的变量名，默认为：m_ddyx"}]
# [param: {"required":false,"key":"mrconli.ddyx.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.ddyx.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.ddyx.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]
# [param: {"required":false,"key":"mrconli.ddyx.is_proxy","bool":true,"placeholder":"","name":"是否启用代理","desc":"开启代理就勾选，其实不需要代理"}]
# [param: {"required":false,"key":"mrconli.ddyx.proxy_pool","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
...
scripts_name =  "铛铛"
full_scripts_name =  "铛铛一下"
bucket_prefix = "mrconli.ddyx"
...
def decode_jwt(token: str) -> dict:
    """解析 JWT 字符串，返回头部 + 载荷（不验证签名）"""
    try:
        # 拆分 JWT
        parts = token.split('.')
        if len(parts) != 3:
            return {"error": "JWT 格式错误，必须是 3 段"}

        # 安全的 Base64 URL 解码（修复版）
        ...
        # 解析 payload 中的 mobile 和 appId
        payload = json.loads(safe_b64decode(parts[1]))
        print(payload)
        mobile = payload.get('mobile')
        openid = payload.get('openid')
        appId = payload.get('appId')
        if mobile:
            return True, mobile, appId
        else:
            return True, openid, appId
    except Exception as e:
...
def get_headers(token):
    ...
    获取请求头
    """
    success, mobile, appId = decode_jwt(token)
    if not success:
        return None
    ddyx_headers = {
    ...
    zfb_alipayHeaders = {
        ...
        'alipayminimark': 'i/PofbtYH5bQuPIsFYS0NVZY5LSVWxzxcJbDfdt1hoZ8G3W96cu6k6zSq3lDUMwZ2ZDawThWif7hkNKazfkDxUu5FsxSO//E4eQk7nAVagk=',
        'referer': 'https://2018120362447108.hybrid.alipay-eco.com/2018120362447108/0.2.2512151439.1/index.html#pageMine/pages/walletWithdraw/walletWithdraw?__appxPageId=2&channelId=147',
        'token': token,
        'x-release-type': 'ONLINE',
    }
    if appId == 'wxe378d2d7636c180e' or appId == 'wxbe2f25800165b6f8':
        return ddyx_headers
    else:
        return zfb_alipayHeaders

def get_user_info(token):
    headers = get_headers(token)
    url = 'https://vues.dd1x.cn/api/h/get_account_detailed'
    response = requests.get(url, headers=headers)
    {'code': 0, 'msg': '', 'data': {'total': 0.0, 'red_envelope': 0.0, 'recovery_amount': 0.0, 'accumulated': 0.11, 'unauditedAmount': 0.0}}
    if response.json()['code'] == 0:
        red_envelope = response.json()['data']['red_envelope']
        return True, red_envelope
    else:
        return False, None
...
def batch_login():
    ...
    for index, account in enumerate(accounts, 1):
        try: 
            success, mobile, appId = decode_jwt(account)
            if not success:
                fail_count += 1
                error_reasons.append(f"❌ {account} ck解析失败")
                continue
            if appId == "wxe378d2d7636c180e":
                type = "ddyx"
                type_name = "铃铛一下"
            elif appId == "wxbe2f25800165b6f8":
                type = "kb"
                type_name = "快宝"
            elif appId == "2018120362447108":
                type = "zfb"
                type_name = "支付宝"
            else:
                fail_count += 1
                error_reasons.append(f"❌ {account} 小程序未知")
                continue
            success, red_envelope = get_user_info(account)
            if success:
                phone = f"{mobile}_{type}"
                success_count += 1
                new_token = f"{account}#{type}#{mobile}"
                middleware.bucketSet(f'{bucket_prefix}.token', phone, new_token)
            
                current_accounts = eval(middleware.bucketGet(f'{bucket_prefix}.user', userid) or '[]')
                if phone not in current_accounts:
                    add_count += 1
                    status = f"✅ {mask_phone(mobile)} [{type_name}] 登录成功"
                    current_accounts.append(phone)
                    middleware.bucketSet(f'{bucket_prefix}.user', userid, json.dumps(current_accounts, ensure_ascii=False))
                else:
                    update_count += 1
                    status = f"✅ {mask_phone(mobile)} [{type_name}] 更新成功"
                    accountVip = middleware.bucketGet(f'{bucket_prefix}.auth', phone)
                    if not accountVip or accountVip < today_time:
                        sender.reply(f"⚠️ 账号未授权或授权已过期，环境变量未提交青龙...")
                    else:
...
def query():
    ...
    if len(accounts) > 1:
        ...
        for idx, acc in enumerate(accounts, 1):
            ...
            mobile, type = acc.split('_')
            type_name = types.get(type)
            menu += f"[{idx}] {mask_phone(mobile)} [{type_name}] \n"
        menu += "====================\n⚠️ 请回复数字序号(输入q退出)\n💡 支持多选，如：1,3,4,7"
        sender.reply(menu)
    ...
    for account in target_accounts:
        try:
            ...
            if not accountVip:
                ...
                mobile = parts[2]
                type_name = types.get(type)
                success, red_envelope = get_user_info(m_token)
                if not success:
        ...
        for user in users:
            ...
            for account in accounts:
                try:
                    ...
                    success, red_envelope = get_user_info(m_token)
                    if not success:
                        notify_user(user, account, f"账号 {account} [{type_name}] ck检测失效")
                        continue
                except Exception as e:
...
def get_config():
    """获取插件配置"""
    try:
        var_name = middleware.bucketGet(bucket_prefix, 'var_name') or "m_ddyx"
        if not var_name:
            print("未配置变量名，使用默认值: m_ddyx")
            var_name = 'm_ddyx'
...
def show_account_menu(account):
    ...
    menu = f"""
=====账号操作=====
📱 账号: {mask_phone(mobile)} [{type_name}]
...
def tutorial():
    ...
    tutorial_text = (
        ...
        "📝 入口:\n"
        f"    #小程序://快宝旧衣服回收/HDF2wAnNbdOImht\n"
        f"    #小程序://旧衣服回收/hYPOTBT9PFPskEy\n"
        f"    支付宝小程序：铛铛一下\n"
        "🌟 基础指令:\n"

...
#[rule: ^小米登录$|^登录小米$|^小米查询$|^小米管理$|^小米清理$|^小米一键更新$|^小米授权$|^小米兑换$] 匹配规则，多个规则时向下依次写多个
#[cron: 0 0 0 0 0] cron定时，支持5位域和6位域
#[priority: 0] 优先级，数字越大表示优先级越高
#[platform: qq,qb,wx,tb,tg,web,wxmp] 适用的平台
#[open_source: false]是否开源
#[icon: https://img-cf.885666.xyz/fc6f139aa63c28af6e25ece264481322.png]图标链接地址，请使用48像素的正方形图标，支持http和https
#[version: 4.4]版本号
#[public: true] 是否发布？值为true或false，不设置则上传aut云时会自动设置为true，false时上传后不显示在市场中，但是搜索能搜索到，方便开发者测试
#[price: 18.88] 上架价格
#[description: 功能：小米社区签到等级|钱包视频会员(2.5天)|视频会员兑换<br>指令：小米登录|登录小米|小米查询|小米管理|小米清理|小米一键更新|小米授权|小米兑换<br>小米社区登录插件，未内置脚本，购买后联系管理员获取<br>1.0.1：新增查询功能<br>1.0.2：增加Cookie更新指令<br>1.0.4：增加账号需要验证的判断处理<br>1.0.5：增加扫码登录以便过安全验证<br>1.0.6：增加扫码登录超时判断,修复状态检测死循环<br>1.0.7：强制绑定账密<br>尝试修复登录过程中的一些bug<br>1.0.8：修复登录过程中的一些bug<br>1.1.0：新增易支付付款方式<br>4.4：修复抢兑提交青龙变量的更新判断逻辑] 使用方法尽量写具体
# [param: {"required":true,"key":"s_xiaomi.zsm","bool":false,"placeholder":"必填项,http://xxxx.co/xxx.jpg","name":"收款方式","desc":"Wxbot赞赏码/收款码链接"}]
# [param: {"required":true,"key":"s_xiaomi.ql_config","bool":false,"placeholder":"Host丨ClientID丨ClientSecret","name":"设置对接容器","desc":"青龙配置,用丨分割"}]
# [param: {"required":true,"key":"s_xiaomi.var_name","bool":false,"placeholder":"必填项,例:xiaomick","name":"青龙变量名","desc":"提交到青龙的变量名"}]
# [param: {"required":true,"key":"s_xiaomi.price","bool":false,"placeholder":"例:0.88,不填为0元","name":"上车价格","desc":"上车价格(单位:元)/月"}]
# [param: {"required":true,"key":"s_xiaomi.coin","bool":false,"placeholder":"不填为 关闭状态","name":"积分开通","desc":"授权一个月需要多少积分"}]
# [param: {"required":false,"key":"s_xiaomi.autuser","bool":false,"placeholder":"autman账号","name":"autman账号","desc":"部分微信框架无法发送二维码请填入autman账号"}]
# [param: {"required":false,"key":"s_xiaomi.autpwd","bool":false,"placeholder":"autman密码","name":"autman密码","desc":"部分微信框架无法发送二维码请填入autman密码"}]
# [param: {"required":false,"key":"s_xiaomi.ma_pay_switch","bool":true,"placeholder":"true/false","name":"码支付功能","desc":"开启后使用码支付，关闭则使用扫码支付。"}]
...
def get_config():
	var_name=middleware.bucketGet('s_xiaomi','var_name')or'xiaomick';ql_config=middleware.bucketGet('s_xiaomi','ql_config')or'';price=Decimal(middleware.bucketGet('s_xiaomi','price')or'1');coin_config=middleware.bucketGet('s_xiaomi','coin')
	try:
...
def add_to_qinglong(token,account,phone):
	try:
		...
		cookie_str=middleware.bucketGet('s_xiaomi_token',account)
...
def login():
	...
	if accounts:
		account_list='=====已绑定账号=====\n[0] 扫码登录新账号\n'
		for(i,account)in enumerate(accounts,1):auth_time=middleware.bucketGet('s_xiaomi_auth',account)or'未授权';account_list+=f"[{i}] {format_phone(account)} ({auth_time})\n"
		...
		try:
			...
			if 0<=index<len(accounts):
				selected_account=accounts[index];password=middleware.bucketGet('s_xiaomi_pwd',selected_account)
				if not password:
...
def login_with_account(phone,password,skip_auth=False):
	try:
		...
		if api_data.success:
			sender.reply(f"✅ 账号 {format_phone(phone)} 登录成功，正在获取Cookie...");middleware.bucketSet('s_xiaomi_pwd',phone,password);cookies=get_cookies_by_passtk(api_data.user_id,api_data.pass_token,headers['User-Agent'])
...
def qr_login(phone,password=None):
	try:
		...
		sender.reply(f"\n======扫码登录======\n请扫描二维码登录，有效时长2分钟\n==================")
		if middleware.bucketGet('s_xiaomi','autuser')and middleware.bucketGet('s_xiaomi','autpwd'):
			try:
				...
				if qr_response.status_code==200:
					...
					try:
						with open(qr_filename,'rb')as f:files={'imgfile':f};data={'username':str(middleware.bucketGet('s_xiaomi','autuser')),'password':str(middleware.bucketGet('s_xiaomi','autpwd'))};upload_response=requests.post('http://aut.zhelee.cn/imgUpload',data=data,files=files,timeout=10)
		...
		middleware.bucketSet('s_xiaomi_pwd',input_phone,password);accounts=eval(uservalue or'[]')
		if input_phone not in accounts:accounts.append(input_phone);middleware.bucketSet('s_xiaomi_user',userid,str(accounts));uservalue=str(accounts)
		cookie_str=cookie_to_string(login_data['cookies']);new_cookie_str=f"{login_data['user_id']}#{password}#{cookie_str}";middleware.bucketSet('s_xiaomi_token',input_phone,new_cookie_str);result=process_login(login_data['cookies'],input_phone,login_data['user_id'])
...
def process_login(cookies,phone,user_id,skip_auth=False,skip_add_account=False):
	try:
		...
		if not skip_add_account:
			accounts=eval(uservalue or'[]')
			if phone not in accounts:accounts.append(phone);middleware.bucketSet('s_xiaomi_user',userid,str(accounts));sender.reply(f"✅ 已将账号 {phone} 添加到您的账号列表")
		cookie_str=cookie_to_string(cookies);password=middleware.bucketGet('s_xiaomi_pwd',phone)or'None';new_cookie_str=f"{user_id}#{password}#{cookie_str}";middleware.bucketSet('s_xiaomi_token',phone,new_cookie_str);auth_time=middleware.bucketGet('s_xiaomi_auth',phone);current_date=str(datetime.now().date());is_authorized=auth_time and auth_time>current_date
		if skip_auth:
			if is_authorized:
...
def process_auth(account):
	try:
		...
		total_price=price*months;available_payments=[];ma_pay_switch=(middleware.bucketGet('s_xiaomi','ma_pay_switch')or'false').lower()
		if ma_pay_switch=='true':
			...
			zsm=middleware.bucketGet('s_xiaomi','zsm')
...
def process_payment_handle(account,months,payment_type):
	try:
		total_price=price*months;ma_pay_switch=(middleware.bucketGet('s_xiaomi','ma_pay_switch')or'false').lower()
		if ma_pay_switch=='true':
...
def handle_mapay_payment(project,months,money,pay_type=None):
	...
	pay_lock_key='xiaomi_recharge_lock';lock_info=middleware.bucketGet('s_xiaomi',pay_lock_key)
	if lock_info:
		try:
			lock_data=json.loads(lock_info)
			if time.time()-lock_data['time']<120:sender.reply('当前有其他用户正在支付中，请稍后再试!');return False
		except:pass
	lock_data={'user':userid,'time':int(time.time())};middleware.bucketSet('s_xiaomi',pay_lock_key,json.dumps(lock_data))
	try:
...
==================""")
				...
				if choice.lower()=='q':sender.reply('✅ 已取消支付');middleware.bucketDel('s_xiaomi',pay_lock_key);return False
				try:
...
正在创建支付订单...
==================""")
					else:sender.reply('❌ 选择无效，已取消支付');middleware.bucketDel('s_xiaomi',pay_lock_key);return False
				except ValueError:sender.reply('❌ 输入无效，已取消支付');middleware.bucketDel('s_xiaomi',pay_lock_key);return False
		...
		except Exception as e:sender.reply(f"❌ 创建订单时出错: {str(e)}");middleware.bucketDel('s_xiaomi',pay_lock_key);return False
		if not success:sender.reply(f"❌ 创建订单失败: {msg}");middleware.bucketDel('s_xiaomi',pay_lock_key);return False
		...
		if not pay_url:sender.reply('❌ 获取支付链接失败');middleware.bucketDel('s_xiaomi',pay_lock_key);return False
		...
		sender.reply('支付过程中输入"q"可取消支付');is_paid,msg,data=poll_mapi_payment_status(out_trade_no);middleware.bucketDel('s_xiaomi',pay_lock_key)
		if is_paid:return True
		else:sender.reply(f"❌ 支付未完成: {msg}");return False
	except Exception as e:sender.reply(f"❌ 处理支付订单时出错: {str(e)}");middleware.bucketDel('s_xiaomi',pay_lock_key);return False
def authorize_multiple_accounts(accounts,months=None):
	...
	total_price=price*months*len(accounts);available_payments=[];ma_pay_switch=(middleware.bucketGet('s_xiaomi','ma_pay_switch')or'false').lower()
	if ma_pay_switch=='true':
...
def process_authorization_xiaomi(account,months):
	try:
		...
		middleware.bucketSet('s_xiaomi_auth',account,auth_time);global ql_url,ql_token
...
def calculate_auth_time(account,months):
	current=datetime.now().date();auth=middleware.bucketGet('s_xiaomi_auth',account)
...
def query_single_account(account):
	try:
		...
		video_days=get_video_days(cookies);today_video_days=get_today_video_days(cookies);auth_time=middleware.bucketGet('s_xiaomi_auth',account)or'未授权';query_msg=f"""
=====账号信息=====
📱 账号: {format_phone(account)}
👤 UID: {user_info["uid"]}
📅 到期: {auth_time}
...
def query_xiaomi():
	try:
		...
		for(i,account)in enumerate(accounts,1):
			auth_time=middleware.bucketGet('s_xiaomi_auth',account)
...
def clean_xiaomi():
	...
	try:
		global ql_url,ql_token;ql_url,ql_token=init_qinglong();users=middleware.bucketAllKeys('s_xiaomi_user');cleaned=0;today=str(datetime.now().date())
		for user in users:
			accounts=eval(middleware.bucketGet('s_xiaomi_user',user)or'[]');valid=[]
			for account in accounts:
				auth=middleware.bucketGet('s_xiaomi_auth',account)
				if not auth or auth<=today:middleware.bucketSet('s_xiaomi_token',account,'');middleware.bucketSet('s_xiaomi_auth',account,'');middleware.bucketSet('s_xiaomi_pwd',account,'');delete_from_qinglong(account);cleaned+=1
				else:valid.append(account)
			if valid:middleware.bucketSet('s_xiaomi_user',user,str(valid))
			else:middleware.bucketDel('s_xiaomi_user',user)
	...
	try:
		...
		if option=='1':
			if len(selected_accounts)==1:process_auth(selected_accounts[0])
				...
				total_price=price*months*len(selected_accounts);available_payments=[];ma_pay_switch=(middleware.bucketGet('s_xiaomi','ma_pay_switch')or'false').lower()
				if ma_pay_switch=='true':
					ma_pay_type=middleware.bucketGet('dd_sign_config','ma_pay_type')or''
					if not ma_pay_type:ma_pay_type='alipay,wxpay'
					pay_types=[p.strip()for p in ma_pay_type.split(',')if p.strip()]
...
发送"小米查询"查看账号详情''';sender.reply(success_msg)
					...
					ma_pay_switch_local=(middleware.bucketGet('s_xiaomi','ma_pay_switch')or'false').lower()
			...
			for account in selected_accounts:auth_time=middleware.bucketGet('s_xiaomi_auth',account)or'未授权';confirm_msg+=f"- {format_phone(account)} ({auth_time})\n"
			...
			for account in selected_accounts:
				try:
					accounts.remove(account);middleware.bucketSet('s_xiaomi_token',account,'');middleware.bucketSet('s_xiaomi_auth',account,'');middleware.bucketSet('s_xiaomi_pwd',account,'')
					if delete_from_qinglong(account):success_count+=1
				except:continue
			if accounts:middleware.bucketSet('s_xiaomi_user',userid,str(accounts))
			else:middleware.bucketDel('s_xiaomi_user',userid)
...
def single_user_authorize():
	try:
		...
		accounts=eval(middleware.bucketGet('s_xiaomi_user',target_userid)or'[]')
...
def batch_authorize_all():
	try:
		...
		all_accounts=set();users=middleware.bucketAllKeys('s_xiaomi_user')
		for user in users:accounts=eval(middleware.bucketGet('s_xiaomi_user',user)or'[]');all_accounts.update(accounts)
...
def get_or_create_oaid(account):
	"""获取或创建账号的oaid"""
	oaid=middleware.bucketGet('s_xiaomi_oaid',account)
	if not oaid:
		oaid=generate_oaid()
		middleware.bucketSet('s_xiaomi_oaid',account,oaid)
	return oaid
def xiaomi_exchange():
	try:
		accounts=eval(uservalue or'[]')
...
def check_account_authorization(account):
	"""检查账号授权是否有效"""
	auth_time=middleware.bucketGet('s_xiaomi_auth',account)
	if not auth_time or not auth_time.strip():return False,'未授权'
	current_date=str(datetime.now().date())
	if auth_time<=current_date:return False,f"授权已过期 (到期: {auth_time})"
	return True,f"授权有效 (到期: {auth_time})"
def exchange_for_account(account,mode='direct'):
	try:
		...
		sender.reply(f"🔄 正在为账号 {format_phone(account)} 获取兑换信息...\n✅ {auth_msg}");cookie_str=middleware.bucketGet('s_xiaomi_token',account)
...
def upload_exchange_to_qinglong(account,prize_code,prize_name,target_phone):
	try:
		ql_url,ql_token=init_qinglong();cookie_str=middleware.bucketGet('s_xiaomi_token',account)
...
def delete_qinglong_exchange():
	try:
		all_task_ids=middleware.bucketAllKeys('s_xiaomi_dh')or[];user_tasks=[]
		for task_id in all_task_ids:
			task_json=middleware.bucketGet('s_xiaomi_dh',task_id)
			if task_json:
				try:
				...
				if confirm=='y':
					...
					if response.status_code==200:
						...
						if env_id_to_delete:
							...
							if delete_response.status_code==200:middleware.bucketDel('s_xiaomi_dh',task_id);sender.reply('✅ 抢兑任务已删除')
							else:middleware.bucketDel('s_xiaomi_dh',task_id);sender.reply('⚠️ 数据桶已删除，但青龙变量删除失败')
						else:middleware.bucketDel('s_xiaomi_dh',task_id);sender.reply('⚠️ 数据桶已删除，但未找到对应的青龙变量')
...
def update_cookie_by_passtoken(account):
	try:
		...
		new_cookies.update({'passToken':pass_token,'userId':user_id});new_cookie_str=cookie_to_string(new_cookies);updated_cookie_str=f"{user_id}#{password}#{new_cookie_str}";middleware.bucketSet('s_xiaomi_token',account,updated_cookie_str);return True,'更新成功'
	...
	for(i,account)in enumerate(valid_accounts,1):
		try:
			...
			if success:success_count+=1;sender.reply(f"✅ 账号 {format_phone(account)} 更新成功 ({i}/{total})")
				...
				sender.reply(f"⚠️ passtoken更新失败: {msg}，尝试账密更新...");password=middleware.bucketGet('s_xiaomi_pwd',account)
...
def sync_to_qinglong():
	try:
		if not sender.isAdmin():sender.reply('❌ 需要管理员权限');return
		global ql_url,ql_token;ql_url,ql_token=init_qinglong();current_date=str(datetime.now().date());all_accounts=[];authorized_accounts=[];users=middleware.bucketAllKeys('s_xiaomi_user')
		if not users:sender.reply('❌ 没有找到任何用户数据');return
		for user in users:
			try:
				accounts=eval(middleware.bucketGet('s_xiaomi_user',user)or'[]')
				for account in accounts:
					if account not in all_accounts:
						all_accounts.append(account);auth_time=middleware.bucketGet('s_xiaomi_auth',account)
...
def main():
	try:
		...
		if'小米查询'in message:query_xiaomi()
			...
			if not sender.isAdmin():sender.reply('❌ 需要管理员权限');return
			ql_url,ql_token=init_qinglong();all_accounts=set();users=middleware.bucketAllKeys('s_xiaomi_user')
			for user in users:accounts=eval(middleware.bucketGet('s_xiaomi_user',user)or'[]');all_accounts.update(accounts)

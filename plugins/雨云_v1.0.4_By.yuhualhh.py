...
# [author: yuhualhh]
# [service: ]
# [description: <br>❶雨云内置任务插件，自动完成每日签到任务，支持账密登录、管理、查询、授权、检测授权过期以及账密错误推送等功能<br>❷部分功能的实现需自行添加计划任务伪装管理员定时，了解如何添加计划任务请看移动云盘插件介绍，关于指令『雨云检测』与『雨云清理』定时『30 18 * * *』，关于指令『雨云一键运行』定时『0 7,19 * * *』]
...
# [param: {"required":true,"key":"yuhua_yuyun.bingfa","bool":false,"placeholder":"","name":"并发数量","desc":"不填默认20"}]
...
class HSH:
    ...
    def get_slide_verify(self):
        """获取滑块验证码 (调用第三方或自身逻辑)"""
        # 这里尝试调用第三方接口，如果不通则返回空，依靠运气
        try:
            url = "https://txdx.wxtool.de5.net/solve_captcha?aid=2039519451&type=1"
            res = requests.get(url, timeout=30)
            if res.status_code == 200:
                d = res.json()
                if d.get("code") == 200 and d.get("data"):
                    return d["data"].get("ticket"), d["data"].get("randstr")
        except: pass
        return "", ""
...
def get_config():
    ...
    bf_str = middleware.bucketGet('yuhua_yuyun', 'bingfa') or '20'

...
# [class: 工具类]
# [author: a740820]
# [service: 2990775834] 售后联系方式
# [disable:true] 禁用开关，true表示禁用，false表示可用
# [admin: false] 是否为管理员指令
# [rule: ^(芳华|fhwl)(登录|登陆)$|^登(录|陆)(芳华|fhwl)$|^(芳华|fhwl)(查询|管理)$|^(查询|管理)(芳华|fhwl)$|^清理芳华$|^芳华授权$|^芳华上传$]
...
# [price: 0] 上架价格
# [description: 芳华未来账号管理插件<br><br>指令：芳华登录、芳华管理、芳华查询、芳华授权(管理员)<br><br>功能：多账号管理、授权后自动同步青龙、支持积分/微信支付，仅提供账号绑定同步，不自动执行芳华币任务<br>登录格式：手机号#密码]
# 插件参数配置
# [param: {"required":false,"key":"G_SKM.zsm","name":"收款码(全局)","placeholder":"http://example.com/pay.jpg","desc":"微信赞赏码/收款码链接"}]
# [param: {"required":false,"key":"G_fhwl_config.price","name":"月费价格","placeholder":"0","value":"0","desc":"默认0元"}]
# [param: {"required":false,"key":"G_fhwl_config.points_per_month","name":"积分/月","placeholder":"100","value":"100","desc":"一个账号每月所需积分数量"}]
# [param: {"required":false,"key":"G_fhwl_config.ql_config","name":"青龙配置","placeholder":"http://ip:5700丨client_id丨client_secret","desc":"青龙面板地址丨应用ID丨应用密钥"}]
# [param: {"required":false,"key":"G_fhwl_config.ql_envname","name":"青龙环境变量名","placeholder":"fhwl","value":"fhwl","desc":"青龙脚本读取的环境变量名称（对应fhwl.py）"}]
...
# ==================== 常量配置 ====================
# 芳华未来登录接口
BASE_LOGIN_URL = "https://api.cdwjyyh.com/app/app/login"
HEADERS = {
    "Host": "api.cdwjyyh.com",
    "Connection": "keep-alive",
    "content-type": "application/json",
    "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 Html5Plus/1.0 (Immersed/20) uni-app",
}
# 登录提示：手机号#密码
loginMessage = """
=====芳华未来登录绑定=====
...
def get_config():
    """动态获取插件配置"""
    try:
        price_str = middleware.bucketGet(bucket="G_fhwl_config", key="price") or "0.88"
        ...
        points_per_month_str = (
            middleware.bucketGet(bucket="G_fhwl_config", key="points_per_month") or "100"
        )
        points_per_month = (
            int(points_per_month_str) if points_per_month_str.isdigit() else 100
        )
        ql_config = middleware.bucketGet(bucket="G_fhwl_config", key="ql_config") or ""
        ql_envname = (
            middleware.bucketGet(bucket="G_fhwl_config", key="ql_envname") or "fhwl"
        )
        return {
            "price": price,
            "zsm": zsm,
...
def get_user_accounts(user_id=None):
    ...
    uservalue = middleware.bucketGet("G_fhwl_user", target_userid) or "[]"
    user_accounts = []
    if uservalue:
        try:
...
def validate_account(phone, pwd):
    """验证手机号密码是否有效，拉取用户信息（仅校验，不执行任务）"""
    try:
        headers = {
        ...
        response = requests.post(
            BASE_LOGIN_URL, headers=headers, json=payload, timeout=15, verify=False
        )
        data = response.json()
...
def add_or_update_ql_env(
    ...
    """添加或更新青龙环境变量，值格式：手机号#密码（匹配fhwl.py读取规则）"""
    ...
    try:
        ...
        for e in envs:
            if e.get("name") == env_name:
                ...
                if f"芳华未来:{account_id}" in env_remarks:
                    existing = e
                    break
        if existing:
...
def sync_to_qinglong(account_id):
    """单个账号同步到青龙（授权后自动调用，变量值：手机号#密码）"""
    config = get_config()
    ql_config_str = config.get("ql_config", "")
    ql_envname = config.get("ql_envname", "fhwl")
    ...
    acc_data_str = middleware.bucketGet("G_fhwl_acc", account_id)
    if not acc_data_str:
        return {"success": False, "reason": "账号手机号密码不存在"}
    acc_info = json.loads(acc_data_str)
    phone = acc_info["phone"]
    pwd = acc_info["pwd"]
    env_value = f"{phone}#{pwd}"
    auth_data_str = middleware.bucketGet("G_fhwl_auth", account_id)
    expire_date = ""
    acc_userid = userid
    if auth_data_str:
        try:
            ...
            pass
    remarks = f"芳华未来:{account_id}|用户:{acc_userid}|到期:{expire_date}"
...
def login():
    ...
    if len(parts) != 2:
        sender.reply("⚠️ 格式错误！正确格式：手机号#密码，无需填写备注")
        return
    phone, pwd = parts[0].strip(), parts[1].strip()
    if len(phone) != 11 or not phone.isdigit():
        sender.reply("⚠️ 手机号格式不正确，请输入11位数字手机号")
        return
    user_info = validate_account(phone, pwd)
    if user_info:
        # 自动使用脱敏手机号作为账号唯一标识，不再手动备注
        mask_p = mask_phone(phone)
        account_id = mask_p
        save_account_info(account_id, phone, pwd)
    else:
        sender.reply(f"⚠️ 手机号或密码错误，登录校验失败，请重新检查")
        return
def save_account_info(account_id, phone, pwd):
    """保存账号信息，account_id为脱敏手机号"""
    accounts = get_user_accounts()
    if account_id not in accounts:
        accounts.append(account_id)
        middleware.bucketSet("G_fhwl_user", userid, json.dumps(accounts))
    middleware.bucketSet("G_fhwl_acc", account_id, json.dumps({"phone":phone, "pwd":pwd}))
    mask_p = mask_phone(phone)
    success_msg = f"""
=====绑定成功=====
📱 账号手机号: {mask_p}
...
def query_accounts():
    ...
    accounts = get_user_accounts()
    if not accounts:
        sender.reply("❌ 您尚未绑定任何芳华未来账号，请先使用「芳华登录」绑定")
        return
    account_info_list = []
    ...
    final_msg = "=====芳华账号信息汇总=====" + "".join(account_info_list) + "\n"
    sender.reply(final_msg)
def query_accounts_for_item(account, today):
    """单个账号详情查询"""
    acc_data_str = middleware.bucketGet("G_fhwl_acc", account)
    if not acc_data_str:
        return None
    acc_info = json.loads(acc_data_str)
    phone = acc_info["phone"]
    ...
    if user_info:
        ...
        auth_data_str = middleware.bucketGet("G_fhwl_auth", account)
...
def manage():
    accounts = get_user_accounts()
    if not accounts:
        sender.reply("❌ 您尚未绑定任何芳华未来账号，请先绑定")
        return
    authorized_count = 0
    unauthorized_accounts = []
    for account_id in accounts:
        auth_data = middleware.bucketGet("G_fhwl_auth", key=account_id)
        if auth_data:
            try:
...
def complete_authorization(account_id, months):
    """完成授权并记录"""
    existing_auth = middleware.bucketGet("G_fhwl_auth", account_id)
    ...
    middleware.bucketSet(
        bucket="G_fhwl_auth", key=account_id, value=json.dumps(auth_data)
    )
    # 授权成功后自动上传青龙
...
==================""")
    ...
    try:
        ...
        middleware.bucketDel(bucket="G_fhwl_acc", key=account_id)
        middleware.bucketDel(bucket="G_fhwl_auth", key=account_id)
        if account_id in accounts:
            accounts.remove(account_id)
            if accounts:
                middleware.bucketSet(
                    bucket="G_fhwl_user", key=userid, value=json.dumps(accounts)
                )
            else:
                middleware.bucketDel(bucket="G_fhwl_user", key=userid)
...
def upload_to_qinglong():
    ...
    # 收集已授权账号
    authorized_accounts = []
    auth_keys = middleware.bucketAllKeys(bucket="G_fhwl_auth") or []
    for account_id in auth_keys:
        auth_data_str = middleware.bucketGet("G_fhwl_auth", key=account_id)
        if not auth_data_str:
            continue
        try:
            auth_data = json.loads(auth_data_str)
...
def admin_authorize_account():
    ...
    choice = sender.input(60000, 1, False)
    if choice == "1":
        users = middleware.bucketAllKeys(bucket="G_fhwl_user")
        if not users:
        ...
        if not accounts:
            sender.reply(f"❌ 用户 {target_userid} 未绑定任何芳华未来账号")
            return
        account_lines = []
        for i, account_id in enumerate(accounts, 1):
...
def clear_accounts():
    ...
    accounts = get_user_accounts()
    if not accounts:
        sender.reply("❌ 您没有绑定任何芳华未来账号")
        return
    sender.reply(f"""
=====清理芳华未来数据=====
⚠️ 警告：此操作将清除您的所有芳华未来账号数据
...
try:
    ...
    usermessage = ""
if re.search(r"芳华登录|芳华登陆", usermessage):
    login()
elif re.search(r"芳华管理", usermessage):
    manage()
elif re.search(r"芳华查询", usermessage):
    query_accounts()
elif re.search(r"芳华上传", usermessage):
    upload_to_qinglong()
elif re.search(r"芳华教程", usermessage):
    sender.reply(
        "=====芳华未来使用教程=====\n"
        "1. 「芳华登录」绑定账号\n"
        "   格式：手机号#密码\n"
        "2. 「芳华管理」进行账号授权\n"
        "   授权成功后自动同步到青龙面板fhwl环境变量\n"
        "3. 「芳华查询」查看账号状态\n"
        "4. 「芳华上传」手动上传到青龙(管理员)\n"
        "===================="
    )
elif re.search(r"芳华授权$", usermessage) and sender.isAdmin():
    admin_authorize_account()
elif re.search(r"清理芳华|芳华清理", usermessage):
    clear_accounts()
else:
    sender.setContinue()
...
//[class: 工具类]
//[public: true] 
//[icon: http://www.icosky.com/icon/png/System/Boomy/Group%20of%20users.png]
//[price: 2] 
//[version: 1.0.5] 
//[admin: true] 
...
//[priority: 9999999999]
//[service: 97393412]
//[description: 多渠道通知推送<br/>兼容最新版本奥特曼,需要给[qinglong数据]权限<br/>操作入口:后台系统管理-插件权限-qinglong和jdNotify权限[打开]<br/> 7-5 增加获取实时CK开关，获取青龙内的PIN在获取jdNotify进行判断是否有效<br/>插件交流群：858456556]
// [param: {"required":true,"key":"who_tong.jcql","placeholder":"容器名称","name":"青龙名称","desc":"获取名方法:奥特曼后台-容器管理--对接容器--名称"}]
// [param: {"required":true,"key":"who_tong.jczh","placeholder":"","name":"通知渠道","desc":"wb,qq"}]
// [param: {"required":false,"key":"who_tong.pro_yxkg","bool":true,"placeholder":"false","name":"CK有效","desc":"默认不开启CK有效时停止登录,需要禁止有效CK继续登录则需打勾"}]
// [param: {"required":true,"key":"who_tong.jcsx","placeholder":"","name":"失效提示语","desc":"该账号已经失效，如需继续挂京豆请发送：登录"}]
...
var stxs = bucketGet("who_tong", "jczh")
...
function mian() {
  ...
  let jc_yx = 0
  let jc_sx = 0
  let containerEnv, container
  var ql_name = bucketGet("who_tong", "jcql")
  ...
  for (let sa = 0; sa < cookieObj.length; sa++) {
    let pina = cookieObj[sa].value.match(/(?<=pt_pin=)[^;]+/g)
    var qd = stxs.split(",")
    if (pro_yxkg == "" || pro_yxkg == "false") {
      var jd_ckck = cookieObj[sa].value
    } else {
      var jnStr = bucketGet("jdNotify", pina)
      jn = JSON.parse(jnStr)
      var jd_ckck = "pt_key=" + jn.PtKey + ";pt_pin=" + jn.ID + ";"
    }

    if (jdd_ck(jd_ckck)) {
      Debug(`${pina},有效,` + sa)
      if (cookieObj[sa].status == 1) {
        container.ApiQL("envs/enable", "", "put", `[${cookieObj[sa].id}]`)//启用envs/enable
      }
      //有效账号，不推送
      jc_yx = jc_yx + 1
    } else {
      ...
      if (cookieObj[sa].status == 1) {
        ...
        container.ApiQL("envs/disable", "", "put", `[${cookieObj[sa].id}]`)//禁用envs/disable
      }
      ...
      if (GetContent == "失效禁用") {
        ...
        if (pin_ck == "") {
          for (let k = 0; k < qd.length; k++) {
            ...
            var bbk_rlyz = bucketGet("who_tong", "jcsx")
            if (bbk_rlyz == "") {
              TXfs_tuisong(qd[k], bind, `===JD账号失效通知===\n东东账号：${decodeURIComponent(pina)}\n该账号已经失效，如需继续挂京豆请发送：登录`)
            } else {
              TXfs_tuisong(qd[k], bind, `===JD账号失效通知===\n东东账号：${decodeURIComponent(pina)}\n${bbk_rlyz}`)
            }
            sleep(8000)
  ...
  notifyMasters(`[${GetContent}]检查账号-渠道推送数据\nCK数量:${cookieObj.length},有效:${jc_yx},失效:${jc_sx}`)
}
...
function duorong() {
  allpins = ["CX", "J3", "KL", "QP", "J2"]
  allpins.forEach(item => {
    //sendText(item)
    let containerData = qls(item)
    // 获取容器对象
    ...
    try {
      ...
      console.log(`容器：${item},CK数：${cookieObj.length}`)
      for (let sa = 0; sa < cookieObj.length; sa++) {
        let cookie = cookieObj[sa].value
        if (jdd_ck(cookie)) {

        } else {
          var pina = cookie.match(/(?<=pt_pin=)[^;]+/g)
          console.log(`账号失效：${pina}`)
          if (cookie.status == 1) {
      ...
      // notifyMasters("【捕鱼助力】链接容器【" + item + "】失败，请检查配置是否正确或网络是否正常")
      //   return
    }

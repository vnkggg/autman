...
# [rule: ^回收猿(.*)$] 匹配规则，多个规则时向下依次写多个
# [cron: 32 8,16 * * *] cron定时，支持5位域和6位域
# [priority: 55] 优先级，数字越大表示优先级越高
# [platform: all] 适用的平台
# [open_source: false]是否开源
# [icon: https://bbs.autman.cn/assets/files/2025-09-07/1757242448-823459-hsy.webp]图标链接地址，请使用48像素的正方形图标，支持http和https
# [version: 1.4.0]版本号
# [public: true] 是否发布？值为true或false，不设置则上传aut云时会自动设置为true，false时上传后不显示在市场中，但是搜索能搜索到，方便开发者测试
# [price: 5.88] 上架价格
# [description: AI练手，无内置本，无需抓包，ck永续，登录查询授权功能。”<br><br>指令：回收猿（登录|查询|管理|授权|清理|教程）<br>1.4.0更新：修复环境变量提交青龙bug；<br><br>1.3.0更新：插件重构，去除手机号敏感信息，ck需要重新提交，非必要勿更新。<br>1.0.0初版：支持批量登录。] 使用方法尽量写具体
...
# [param: {"required":true,"key":"mrconli.huishouyuan.ql_config","bool":false,"placeholder":"http://xx.xx.xx.xx:xxxx|xxx|xxx","name":"对接青龙","desc":"|"}]
# [param: {"required":false,"key":"mrconli.huishouyuan.var_name","bool":false,"placeholder":"hsy_username","name":"环境变量名","desc":"青龙容器内的变量名，默认为：hsy_username"}]
# [param: {"required":false,"key":"mrconli.huishouyuan.price","bool":false,"placeholder":"1","name":"上车价格","desc":"上车价格(单位:元)/30天"}]
# [param: {"required":false,"key":"mrconli.huishouyuan.coin","bool":false,"placeholder":"不填为关闭状态","name":"积分开通","desc":"授权一个月的积分，只能为整数"}]
# [param: {"required":false,"key":"mrconli.huishouyuan.coin_bucket","bool":false,"placeholder":"","name":"积分数据桶","desc":"默认使用dd_sign_points"}]
# [param: {"required":false,"key":"mrconli.huishouyuan.is_proxy","bool":true,"placeholder":"","name":"是否启用代理","desc":"开启代理就勾选，其实不需要代理"}]
# [param: {"required":false,"key":"mrconli.huishouyuan.proxy_pool","bool":false,"placeholder":"http://代理池API地址","name":"代理池地址","desc":"代理API服务地址"}]
...
def get_info(username):
    ...
    try:
        response = _send_request('GET', url, params=params, headers=headers).json()
        mobile = response['data']['mobile']       #  手机号
        passport = response['data']['passport']    # 昵称
        print(f"昵称：{passport}，手机号：{mobile}")
        return True, mobile, passport
    except Exception as e:
        print(f'网络请求异常：{e}')
...
def get_config():
    """获取插件配置"""
    try:
        var_name = middleware.bucketGet(bucket_prefix, 'var_name') or "hsy_username"
        if not var_name:
...
def perform_batch_auth(selected_accounts, days=None):
    ...
    if days is None:
        ...
        if not days_input:
            sender.reply("❌ 操作超时")
            return
        elif days_input == 'q' or days_input == 'Q':
            sender.reply("✅ 已取消授权")
            return
        try:
            days = int(days_input)
...
回复数字选择方式"""
        ...
        # 微信支付处理
        if pay_choice == '1':
            amount = price * (Decimal(days) / 30) * len(selected_accounts)
        ...
        if '-' in choice:
            ...
            for range_str in ranges:
                if '-' in range_str:
                    ...
                    c_num = int(range_str)
                    if c_num < 1 or c_num > len(accounts):
                        sender.reply(f'❌ 选择超出范围：{c_num}')
                        return
                    target_accounts.append(accounts[c_num - 1])
...
回复"q"退出""")
        ...
        for account in target_accounts:
            try:
                ...
                sender.reply(f"❌ 账号 {mask_phone(account)} 授权失败: {str(e)}")
                log_operation('auth', user_id, account, 'failed', str(e))
...
==================""")
            ...
            sender.reply(f"✅ 已成功授权 {success_count}/{len(target_accounts)} 个账号 {days}天")
...
def tutorial():
    ...
    tutorial_text = (
        ...
        "📝 入口:\n"
         ...
         "    [CQ:image,file=https://bbs.autman.cn/assets/files/2026-05-03/1777772546-550270-qq20260503-094145.png]\n"
        "-------------------\n"
        "🌟 基础指令:\n"
